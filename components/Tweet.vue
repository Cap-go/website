...
function messageFormatter(message: string) {
  // find all @mentions and make it in a span with class text-blue-400
  return message.replace(/(@\w+)/g, '<span class="text-blue-400">$1</span>').replace('\n', '<br/>')
}
</script>

<template>
  <div
    class="p-4 mx-auto md:mx-3 mt-0 mb-6 max-w-80 md:max-w-72 align-baseline border border-solid border-zinc-700 bg-gray-800"
  ...
  >
    <div class="flex justify-between p-0 m-0 align-baseline border-0 border-solid">
      <div
        class="grid gap-3 items-center p-0 mx-0 mt-0 mb-3 align-baseline border-0 border-solid"
        style="grid-template-columns: auto 1fr;"
      >
        <picture class="border-0 border-solid">
          <img
            height="44" width="44" loading="lazy"
        ...
        <div class="p-0 m-0 align-baseline border-0 border-solid">
          <p
            class="p-0 m-0 text-base font-normal tracking-normal leading-relaxed text-gray-200 align-baseline border-0 border-solid"
          >
            <span class="p-0 m-0 font-semibold align-baseline border-0 border-solid">{{ tweet.name }}</span>
          </p>
          <p
            class="p-0 m-0 text-base font-normal tracking-normal leading-relaxed align-baseline border-0 border-solid text-zinc-400"
          >
            @{{ tweet.handle }}
          </p>
        </div>
      ...
      <svg
        viewBox="0 0 20 20" fill="none" width="20" height="20" role="img" color="#03A9F4" size="20"
        class="block align-middle border-0 border-solid text-sky-500"
      >
        ...
        <path
          ...
          fill="#03A9F4" class="border-0 border-solid"
        />
      </svg>
    </div>
    <p
      class="p-0 m-0 text-base font-normal tracking-normal leading-relaxed text-gray-200 align-baseline whitespace-pre-line border-0 border-solid"
    >
      <span
        class="p-0 m-0 align-baseline border-0 border-solid"
        v-html="messageFormatter(tweet.message)"
      />
    </p>
  </div>
</template>
