...
const description = m.top_react_native_apps({}, { locale: props.locale })

const apps = ref<any[]>([])

const others = ref(['top_capacitor_app', 'top_flutter_app', 'top_cordova_app'])

fetch(`${config.public.baseApiUrl}/private/store_top?mode=reactNative`).then((res) => {
  if (res.ok) {
    res.json().then((data) => {
  ...
  <section class="py-10 sm:py-12 lg:py-20">
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      <div class="max-w-2xl mx-auto text-center">
        <h1 class="text-3xl font-bold leading-tight text-white sm:text-4xl lg:text-5xl">{{ m.top_react_native_apps_title({}, { locale: props.locale }) }}</h1>
        ...
        <p class="max-w-xl mx-auto mt-4 text-xs leading-relaxed text-gray-200">
          {{ m.react_native_power_aproximately_1_of_apps_on_google_play_store({}, { locale: props.locale }).replace('$1', usage.toString()) }}
        </p>
      </div>
