...
const description = m.top_native_script_apps({}, { locale: props.locale })
...
fetch(`${config.public.baseApiUrl}/private/store_top?mode=nativeScript`).then((res) => {
  if (res.ok) {
    res.json().then((data) => {
  ...
  <section class="py-10 sm:py-12 lg:py-20">
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      <div class="max-w-2xl mx-auto text-center">
        <h1 class="text-3xl font-bold leading-tight text-white sm:text-4xl lg:text-5xl">{{ m.top_native_script_apps_title({}, { locale: props.locale }) }}</h1>
        ...
        <p class="max-w-xl mx-auto mt-4 text-xs leading-relaxed text-gray-200">
          {{ m.native_script_power_aproximately_1_of_apps_on_google_play_store({}, { locale: props.locale }).replace('$1', usage.toString()) }}
        </p>
      </div>
