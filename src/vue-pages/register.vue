    ...
    <div class="relative w-full max-w-4xl">
      ...
      <div class="relative z-10 grid overflow-hidden bg-white rounded-lg shadow-xl md:grid-cols-2">
        <div class="p-6 space-y-6">
          <div>
            <h2 class="text-4xl font-bold text-gray-900">{{ m.register_title({}, { locale: props.locale }) }}</h2>
            <p class="text-sm text-gray-500">
              {{ m.already_have_account({}, { locale: props.locale }) }}
              <a href="https://web.capgo.app/login/" target="_blank" class="text-blue-500 hover:underline">{{ m.sign_in({}, { locale: props.locale }) }}</a>
            </p>
          </div>
          <form @submit.prevent="handleSubmit" class="space-y-4 text-black">
            <div>
              <label for="email" class="block text-sm font-medium text-gray-700">{{ m.email_label({}, { locale: props.locale }) }}</label>
              <input
                id="email"
                v-model="email"
                type="email"
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label for="firstName" class="block text-sm font-medium text-gray-700">{{ m.first_name_label({}, { locale: props.locale }) }}</label>
                <input
                  id="firstName"
                  v-model="firstName"
                  type="text"
              ...
              <div>
                <label for="lastName" class="block text-sm font-medium text-gray-700">{{ m.last_name_label({}, { locale: props.locale }) }}</label>
                <input
                  id="lastName"
                  v-model="lastName"
                  type="text"
            ...
            <div>
              <label for="password" class="block text-sm font-medium text-gray-700">{{ m.password_label({}, { locale: props.locale }) }}</label>
              <input
                id="password"
                v-model="password"
                type="password"
            ...
            >
              ...
              <template v-else> {{ m.sign_up_button({}, { locale: props.locale }) }} </template>
            </button>
          </form>
          <div class="relative">
            ...
            <div class="relative flex justify-center text-sm">
              <span class="px-2 text-gray-500 bg-white">{{ m.need_help({}, { locale: props.locale }) }}</span>
            </div>
          </div>
          <a
          ...
          >
            ...
            {{ m.open_support({}, { locale: props.locale }) }}
          </a>
        </div>
        <div class="flex items-center p-4 bg-slate-800">
          <blockquote class="text-white">
            <p class="mb-4 text-2xl font-bold">
              {{ m.testimonial_title({}, { locale: props.locale }) }}<br />
              After update,
              <span class="px-2 py-1 text-white bg-orange-500 rounded-full">{{ m.testimonial_highlight({}, { locale: props.locale }) }}</span>
            </p>
            <div class="flex items-center mb-4">
              <img src="/avatar-male-2.webp" alt="User" class="w-12 h-12 mr-4 rounded-full" />
              <div>
                <cite class="font-bold text-white">Jermaine</cite>
                <p class="text-gray-400">{{ m.testimonial_description({}, { locale: props.locale }) }}</p>
              </div>
            </div>
          </blockquote>
        </div>
