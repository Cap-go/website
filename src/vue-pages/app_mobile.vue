...
<template>
  <section class="font-fira-sans mx-4 lg:max-w-[85%] xl:mx-auto">
    <div class="flex justify-between md:my-20">
      <div class="w-full md:w-1/2">
        <div class="pr-4">
          ...
          <p class="mt-8 text-lg text-gray-300">
            {{ m.run_your_projects_on_your_own_device_faster_than_ever_and_share_those_projects_across_your_whole_team() }}
          </p>
          <p class="mt-10 mb-20 text-sm text-gray-300">{{ m.test_version_in_the_sandbox_app_manage_version_and_channel() }}</p>
        </div>
        <div class="flex items-center space-x-10">
          <span class="flex items-center rounded-[1.125rem] bg-[#EAEAEA] px-4 py-1">
            <svg class="h-10 text-pumpkinOrange-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            ...
            <p class="pl-4 text-black">{{ m.speed() }}</p>
          </span>
          <span class="flex items-center rounded-[1.125rem] bg-[#EAEAEA] px-4 py-1">
            <svg class="h-10 text-pumpkinOrange-500" fill="currentColor" viewBox="0 0 16 18" xmlns="http://www.w3.org/2000/svg">
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
            ...
            <p class="pl-4 text-black">{{ m.security() }}</p>
          </span>
          <span class="flex items-center rounded-[1.125rem] bg-[#EAEAEA] px-4 py-1">
            <svg class="h-10 text-pumpkinOrange-500" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
              <path
            ...
            <p class="pl-4 text-black">{{ m.simplicity() }}</p>
          </span>
        </div>
        <div class="mt-8 mb-16">
          <p class="text-sm text-[#bfbfbf]">{{ m.start_building_rich_experiences_with_your_device_and_your_computer() }}</p>
        </div>
      ...
      <div class="justify-end hidden w-1/2 space-x-8 md:flex">
