...
const description = translations['top_flutter_apps'][props.locale]
...
fetch(`${config.public.baseApiUrl}/private/store_top?mode=flutter`).then((res) => {
  if (res.ok) {
    res.json().then((data) => {
  ...
  <section class="py-10 sm:py-12 lg:py-20">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="mx-auto max-w-2xl text-center">
        <h1 class="text-3xl font-bold leading-tight text-white sm:text-4xl lg:text-5xl">{{ translations['top_flutter_apps_title'][props.locale] }}</h1>
        ...
        <p class="mx-auto mt-4 max-w-xl text-xs leading-relaxed text-gray-200">
          {{ translations['flutter_power_aproximately_1_of_apps_on_google_play_store'][props.locale].replace('$1', usage.toString()) }}
        </p>
      </div>
