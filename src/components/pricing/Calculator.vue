...
const suggestion = computed(() => {
  if (
    mau.value <= solo.mau &&
    storage.value <= solo.storage &&
    bandwidth.value <= solo.bandwidth
  )
    return "Solo";
  else if (
    mau.value <= maker.mau &&
    storage.value <= maker.storage &&
    bandwidth.value <= maker.bandwidth
  )
    return "Maker";
  else if (
    mau.value <= team.mau &&
    storage.value <= team.storage &&
    bandwidth.value <= team.bandwidth
  )
    return "Team";
...
const totalPrice = computed(() => {
  const mauPrice =
    mau.value > props.paygBase.mau
      ? (mau.value - props.paygBase.mau) * props.paygUnits.mau
      : 0;
  const storagePrice =
    storage.value > props.paygBase.storage
      ? (storage.value - props.paygBase.storage) * props.paygUnits.storage
      : 0;
  const bandwidthPrice =
    bandwidth.value > props.paygBase.bandwidth
      ? ((bandwidth.value - props.paygBase.bandwidth) / 1000) *
        props.paygUnits.bandwidth
      : 0;
  ...
  if (sum > 0) return roundNumber(basePrice + sum) * (props.yearly ? 12 : 1);
    ...
    return (
      (suggestion.value
        ? roundNumber(
            props.pricing.find((plan) => plan.name === suggestion.value)!
              .price_m
          )
        : basePrice) * (props.yearly ? 12 : 1)
    );
});
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
      <div class="max-w-2xl mx-auto text-center xl:max-w-4xl">
        ...
        <p class="mt-6 text-sm font-normal text-gray-600 font-pj">
          Enter your estimated monthly active users, updates per month and
          update size to get your estimated monthly cost.
        </p>
      </div>
      ...
      <div class="relative mt-6 lg:mt-12 lg:max-w-5xl lg:mx-auto">
        ...
        <div
          class="relative grid grid-cols-1 px-16 py-12 overflow-hidden text-center text-white bg-gray-900 gap-y-12 md:grid-cols-3 rounded-2xl gap-x-20"
        >
          ...
          <div class="col-span-1 md:col-span-3 flex flex-col items-center">
            <p
              class="mt-5 text-md font-bold tracking-widest text-white uppercase mt-0 font-pj"
            >
              {{ yearly ? "Yearly" : "Monthly" }} Price
            </p>
            <p
              class="break-all text-3xl font-bold text-gray-900 mt-3 font-pj p-2 bg-white rounded-xl"
            >
              {{ totalPrice }}€
            </p>
            <p
              v-show="suggestion"
              class="mt-5 text-sm font-bold tracking-widest text-white mt-0 font-pj"
            >
              We suggest you to choose the
              <button
                class="font-bold underline underline-current text-red-400 uppercase cursor-pointer"
