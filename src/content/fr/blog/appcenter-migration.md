...
La fonctionnalité Live Update fonctionne en utilisant le [SDK Capgo](https://githubcom/Cap-go/capacitor-updater/) installé dans votre application native pour écouter une destination de canal de déploiement particulière lorsqu'une version Web est attribuée à un canal. Destination, cette mise à jour sera déployée sur les appareils utilisateur exécutant des fichiers binaires configurés pour écouter la destination de canal spécifiée.### Connectez-vous à Capgo CLOUD
