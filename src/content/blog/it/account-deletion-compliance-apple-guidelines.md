...
next_blog: ''
---
**Apple richiede che tutte le app sull'[App Store](https://www.apple.com/app-store/) forniscano un'opzione interna per eliminare gli account utente.** Questa politica, in vigore dal 30 giugno 2022, garantisce agli utenti la possibilità di cancellare completamente i loro dati, dando loro maggior controllo sulla privacy. Ecco cosa devi sapere:

-   **Requisiti Chiave**:
    
    -   L'**opzione di eliminazione account** deve essere facile da trovare nelle impostazioni dell'app.
    -   I dati utente devono essere **completamente rimossi**, eccetto dove la conservazione è legalmente richiesta.
...
### Fasi di Eliminazione Richieste
...
Inoltre, utilizzare l'API REST di Sign in with Apple per revocare i token durante il processo di eliminazione dell'account [\[2\]](https://developer.apple.com/news/?id=12m75xbj)[\[3\]](https://www.ketch.com/blog/posts/apple-delete-account-requirement).

Una volta implementati questi passaggi, concentrarsi sulla garanzia che la rimozione dei dati sia allineata a questi requisiti.
...
| Dati di Autenticazione | Rimozione immediata | Richiesta revoca token |
| Dati di Terze Parti | Eliminazione coordinata | Conformità varia per servizio |
| Cronologia Utilizzo | Pulizia completa | Soggetto a regole di conservazione legale |
...
Il test del processo di eliminazione account è essenziale per garantire conformità e funzionalità. Utilizzare strumenti come [Xcode](https://developer.apple.com/xcode/) e gli strumenti di revisione dell'App Store concentrandosi su:

-   **Flusso di Eliminazione**: Confermare che il processo sia intuitivo e facilmente accessibile.
...
Per gli sviluppatori che utilizzano [Capacitor](https://capacitorjs.com/) con Capgo, gli aggiornamenti in tempo reale possono aiutare a gestire rapidamente i problemi di conformità, evitando di attendere l'approvazione dell'App Store. Durante i test, assicurarsi di verificare:

-   Revoca dei token per gli utenti che hanno effettuato l'accesso con Apple.
-   Rimozione completa dei dati da tutti i servizi connessi.
-   Corretta gestione degli abbonamenti attivi.

## Problemi Comuni e Soluzioni

### Sincronizzazione Dati tra Piattaforme

A volte, l'eliminazione dei dati su iOS e Android non si sincronizza correttamente. Questo accade solitamente a causa delle differenze nel modo in cui ogni piattaforma gestisce l'archiviazione e i dati in cache.

Ecco come affrontare i problemi di sincronizzazione:

-   **Gestore Centralizzato di Eliminazione**: Sviluppare un servizio unificato per gestire compiti chiave come:
    
    -   Pulizia del [local storage](https://capgo.app/plugins/capacitor-data-storage-sqlite/)
    -   Eliminazione dello storage sicuro
...
La gestione degli aggiornamenti in tempo reale gioca un ruolo fondamentale nel mantenere la conformità tra piattaforme e plugin. Qui è dove **Capgo** può semplificare il processo di gestione degli aggiornamenti per l'eliminazione degli account.

Ecco come Capgo aiuta:

-   **Rilasci Graduali**: Testare gli aggiornamenti del flusso di eliminazione con un piccolo gruppo prima di distribuirli ampiamente.
-   **Rollback Istantaneo**: Se qualcosa va storto, tornare immediatamente a una versione stabile precedente.
-   **Analisi Aggiornamenti**: Monitorare i tassi di successo per i flussi di eliminazione e individuare problemi di conformità.
...
Durante la progettazione dell'interfaccia utente per l'eliminazione dell'account, tenere presente questi punti:
...
-   **Comunicazione Chiara**: Fornire una spiegazione dettagliata di cosa succede quando un account viene eliminato. Includere informazioni su:
    ...
    -   Eventuali requisiti legali di conservazione dati
...
Per mantenere un'aderenza costante a questi standard, utilizzare strumenti automatizzati per validare l'UI e i processi. Concentrarsi su queste aree critiche:
...
I test automatizzati regolari aiutano a identificare e affrontare potenziali problemi prima che impattino sugli utenti.

### Prevenzione Rischi

Per minimizzare i rischi e garantire operazioni fluide, seguire questi passaggi:

-   **Gestione Inventario Dati**: Mantenere un registro dettagliato di dove sono memorizzati i dati utente. Questo include storage locale, database cloud, servizi di terze parti, sistemi di autenticazione e backup. Verificare che i dati vengano eliminati da tutte queste posizioni.
...
Dal 30 giugno 2022, Apple richiede che tutte le app includano una funzionalità nativa che permetta agli utenti di eliminare completamente i loro account. Di seguito una suddivisione dei requisiti chiave:
...
| **Comunicazione** | Notificare chiaramente agli utenti il processo di eliminazione e le tempistiche. | Includere informazioni sulle politiche di conservazione dati e gli obblighi legali. |

Queste linee guida costituiscono la base per garantire la conformità alle politiche di Apple.
...
-   **Revisione dell'Archiviazione Dati**  
    Controlla tutte le fonti dove vengono archiviati i dati degli utenti e valuta le politiche di conservazione. Assicurati che le connessioni con terze parti siano documentate in modo approfondito.
...
Per soddisfare i requisiti di eliminazione account di Apple, è fondamentale rimanere aggiornati sulle loro linee guida e creare un processo semplice e intuitivo per l'eliminazione dell'account all'interno della tua app. Rivedere regolarmente le Linee Guida di Revisione dell'App Store di Apple, in particolare le sezioni sulla gestione degli account e dei dati utente, è un passo essenziale per gli sviluppatori.
