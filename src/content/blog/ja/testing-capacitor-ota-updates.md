...
| テストタイプ | 焦点領域 | 成功基準 |
|---|---|---|
| アップデート検出 | バージョンチェック | 応答時間約357ms |
| ダウンロードプロセス | [バンドルダウンロード](https://capgo.app/docs/webapp/bundles/) | 5MBバンドルを約114msで |
| インストール | アップデートの適用 | 正常な統合 |
| ロールバック | バージョンの戻し | 正常なロールバック |

Capgoのグローバルネットワークは、平均API応答時間357msで安定したダウンロード速度を維持するのに役立ちます[\[1\]](https://capgo.app/) 。これらのコンポーネントレベルのテストは、システム全体のパフォーマンスを評価するための基礎となります。
...
CapgoのグローバルCDNは、5MBのバンドルを114msで配信し、APIの平均応答時間は357msです。プラットフォームのスマートな差分アップデートにより、変更された部分のみをダウンロードすることで、帯域幅の使用量をさらに削減します[\[1\]](https://capgo.app/) 。
