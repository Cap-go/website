...
title: 'Cuenta de eliminación de conformidad: Directrices de Apple'
description: >-
  Conoce las directrices de eliminación de cuentas de Apple, los requisitos
...
next_blog: ''
---
**Apple requiere que todas las aplicaciones en la [App Store](https://www.apple.com/app-store/) proporcionen una opción dentro de la aplicación para eliminar cuentas de usuario.** Esta política, vigente desde el 30 de junio de 2022, asegura que los usuarios puedan borrar completamente sus datos, dándoles más control sobre su privacidad. Esto es lo que necesitas saber:
...
- **Requisitos Clave**:
    ...
    - Las aplicaciones que usan **"Sign in with Apple"** deben revocar los tokens a través de la API REST de Apple.
- **Para Desarrolladores**:
    
    - Probar el proceso de eliminación para facilidad de uso y eliminación completa de datos.
    - Asegurar que los servicios de terceros también eliminen los datos del usuario.
...
- **Verificación de Cuenta**: Asegura que la identidad del usuario se confirme a través de un código de correo electrónico o SMS.
...
Una vez que estos pasos estén implementados, enfócate en asegurar que la eliminación de datos se alinee con estos requisitos.

### Estándares de Eliminación de Datos

| **Tipo de Datos** | **Requisitos de Eliminación** | **Consideraciones Legales** |
| --- | --- | --- |
| Contenido del Usuario | Eliminación completa | Puede requerirse retención temporal |
...
Si los datos del usuario se almacenan con servicios de terceros, asegúrate de que esos servicios también eliminen los datos. Las industrias con regulaciones estrictas pueden requerir soporte adicional de servicio al cliente para asegurar el cumplimiento [\[2\]](https://developer.apple.com/news/?id=12m75xbj).

Es crucial verificar la adherencia a estos estándares a través de pruebas exhaustivas.

### Requisitos de Pruebas

Las pruebas del proceso de eliminación de cuenta son esenciales para asegurar el cumplimiento y la funcionalidad. Usa herramientas como [Xcode](https://developer.apple.com/xcode/) y herramientas de revisión de App Store para enfocarte en lo siguiente:
...
Para desarrolladores usando Capacitor con Capgo, las actualizaciones en vivo pueden ayudar a abordar problemas de cumplimiento rápidamente, evitando la necesidad de esperar la aprobación de la App Store. Durante las pruebas, asegúrate de verificar:
...
A veces, la eliminación de datos en iOS y Android no se sincroniza correctamente. Esto generalmente ocurre debido a diferencias en cómo cada plataforma maneja el almacenamiento y los datos en caché.

Aquí te explicamos cómo abordar los problemas de sincronización:

- **Gestor de Eliminación Centralizado**: Desarrolla un servicio unificado para gestionar tareas clave como:
...
- **Transmisión de Eventos Multiplataforma**: Utiliza lógica del lado del servidor para enviar eventos de eliminación a todas las sesiones y dispositivos activos, asegurando la consistencia.
...
| Almacenamiento Local | La limpieza de datos puede estar incompleta | Realizar verificaciones de eliminación recursivas |
...
La gestión de actualizaciones en tiempo real juega un papel fundamental en mantener el cumplimiento a través de plataformas y plugins. Aquí es donde **Capgo** puede simplificar el proceso de gestión de actualizaciones de eliminación de cuentas.

Así es como Capgo ayuda:

- **Implementaciones Graduales**: Prueba las actualizaciones del flujo de eliminación con un grupo pequeño antes de implementarlas ampliamente.
- **Reversión Instantánea**: Si algo sale mal, revierte a una versión estable anterior inmediatamente.
...
Según Capgo, las actualizaciones de cumplimiento llegan al 95% de los usuarios dentro de las 24 horas[\[1\]](https://capgo.app). Además, todas las implementaciones están aseguradas con cifrado de extremo a extremo, garantizando la seguridad de los datos.

Para aprovechar al máximo Capgo para las actualizaciones de cumplimiento:

- **Control de Versiones**: Usa canales de actualización separados para probar los flujos de eliminación antes de implementarlos para todos los usuarios.
...
Estos estándares aseguran una experiencia amigable para el usuario mientras se alinean con los protocolos de cumplimiento más amplios.

### Verificaciones Automatizadas de Cumplimiento

Para mantener una adherencia consistente a estos estándares, usa herramientas automatizadas para validar tu UI y procesos. Enfócate en estas áreas críticas:
...
| **Gestión de Tokens** | Revocar tokens como "Sign in with Apple" | Realizar pruebas de integración de API REST |
...
- **Manejo de Errores**: Prepárate para problemas potenciales como:
    ...
    - Errores de revocación de tokens  
        Implementa mecanismos de respaldo para manejar estos escenarios de manera elegante.
- **Monitoreo y Cumplimiento Legal**: Realiza un seguimiento de métricas clave como tasas de éxito de eliminación, tiempos promedio de finalización y cualquier dato restante. Esto ayuda a identificar y resolver problemas rápidamente. Además, asegura el cumplimiento de requisitos legales, especialmente para industrias con regulaciones estrictas. Para aplicaciones en estos sectores, agrega pasos de verificación adicionales, documenta todos los procedimientos minuciosamente y realiza auditorías regulares.
...
| **Manejo de Datos** | Los datos del usuario y la información de la cuenta deben ser completamente eliminados. | Las eliminaciones parciales no cumplen con los estándares de cumplimiento. |
| **Autenticación** | Revocar correctamente los tokens para cuentas de "Sign in with Apple". | Usar la API REST de "Sign in with Apple" para la implementación. |
...
-   **Implementar Flujos de Eliminación Seguros**  
    Establecer procesos para verificar solicitudes de usuarios, revocar tokens y automatizar la eliminación de datos de usuarios.
...
::: faq
### ¿Cómo pueden los desarrolladores asegurarse de que su aplicación cumpla con los requisitos de eliminación de cuenta de Apple?

Para cumplir con las pautas de eliminación de cuenta de Apple, los desarrolladores deben ofrecer a los usuarios una forma simple y clara de eliminar sus cuentas directamente dentro de la aplicación. El proceso debe ser fácil de encontrar, sencillo de seguir y no debe requerir que los usuarios visiten sitios web externos o se comuniquen con equipos de soporte.

Para aquellos que usan Capacitor, herramientas como **Capgo** pueden facilitar el cumplimiento. Capgo permite actualizaciones en tiempo real de tu aplicación, lo que significa que puedes implementar cambios rápidamente - como ajustes en la función de eliminación de cuenta - sin esperar aprobaciones de la tienda de aplicaciones. Al garantizar el cumplimiento, no solo reduces el riesgo de rechazos de aplicaciones sino que también fortaleces la confianza del usuario.
:::

::: faq
### ¿Cómo pueden los desarrolladores garantizar la eliminación adecuada de datos en todas las plataformas mientras evitan problemas de sincronización?
...
Para cumplir con los requisitos de eliminación de cuenta de Apple, es crucial mantenerse actualizado sobre sus pautas y crear un proceso sencillo y fácil de usar para la eliminación de cuentas dentro de tu aplicación. Revisar regularmente las Pautas de Revisión de la App Store de Apple, especialmente las secciones sobre gestión de cuentas y datos de usuarios, es un paso esencial para los desarrolladores.

Si tu aplicación está construida usando Capacitor, herramientas como **Capgo** pueden simplificar el proceso. Capgo ofrece características como actualizaciones en tiempo real y asegura que tu aplicación cumpla con los requisitos de la plataforma de Apple, todo mientras mantiene una experiencia de usuario fluida. Además, las pruebas regulares y el monitoreo son vitales para confirmar el cumplimiento y resolver rápidamente cualquier problema potencial.
:::
