...
Fourth is the `Disable auto downgrade under native` setting. If this is true then it will be inpossible to down grade from a native version. This means that if you have uploaded a `1.2.0` version to the app store or play store and try to set the channel version to `1.1.0` then the update (downgrade) will fail.
...
Let's start at the beggining. After enabling this setting you should see 2 versions instead of 1.
...
For example you could say that you want only 30% of your users to recive the new update. In that case you would set the `A` version to the old version and the `B` version to the new version (the one thay you only want 30% of users to recive)
...
The last setting is the `Enable progressive deploy`. This is simular to AB testing, however the `Percentage of users recving secondary version` will be increaded every 10 minutes by 10% until it reaches 100%.
Progressive deploy is exclusive with AB testing. You can't have both enabled at the same time.
...
To start a new progressive deploy go to the [specific bundle page](TODO) and click on `Channel`. Then click on the channel that has progressive deploy enabled. You should see something like this:
