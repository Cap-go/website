...
---
First the `Default channel` toggle. When enabled, this channel becomes the default for new devices. For a comprehensive explanation of how default channels work, including how to set up platform-specific defaults (one for iOS, one for Android), see the [Default Channel Configuration](/docs/webapp/main-app-page/#default-channel-configuration) section.
