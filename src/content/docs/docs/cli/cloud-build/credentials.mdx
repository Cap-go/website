...
<Aside type="tip" title="Ad-Hoc Builds">
When using `--ios-distribution ad_hoc`, the App Store Connect API key options (`--apple-key`, `--apple-key-id`, `--apple-issuer-id`) are **not required**. See [Ad-Hoc Distribution Mode](/docs/cli/cloud-build/ios/#ad-hoc-distribution-mode) for details.
</Aside>
