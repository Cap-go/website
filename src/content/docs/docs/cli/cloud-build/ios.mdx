...
- `--provisioning-profile`: Path to your downloaded `.mobileprovision` file
- `--apple-profile-name`: The provisioning profile name you set previously
:::
...
| `APPLE_PROFILE_NAME` | Provisioning profile name from [Provisioning profile](#provisioning-profile). Required unless you pass `--apple-profile-name`; otherwise builds fail. |
...
npx @capgo/cli@latest build credentials save \
  ...
  --provisioning-profile './adhoc_profile.mobileprovision' \
  --apple-profile-name your-adhoc-profile-name
```
    ...
    steps:
      ...
      - name: Request iOS ad-hoc build
        ...
        env:
          ...
          APP_STORE_CONNECT_TEAM_ID: ${{ secrets.APP_STORE_CONNECT_TEAM_ID }}
          BUILD_CERTIFICATE_BASE64: ${{ secrets.BUILD_CERTIFICATE_BASE64 }}
          BUILD_PROVISION_PROFILE_BASE64: ${{ secrets.BUILD_PROVISION_PROFILE_BASE64_ADHOC }}
          APPLE_PROFILE_NAME: ${{ secrets.APPLE_PROFILE_NAME }}
