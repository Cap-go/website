...
<Steps>
  1. Follow all the steps from the previous section to find the TestFlight section in AppStore Connect
...
npx @capgo/cli build credentials save \
  --platform ios \
  --apple-team-id UVTJ336J2D \  # <-- Replace with your Apple Team ID from previous step
  --apple-key ./capgo-tutorial/AuthKey_66FGQZB566.p8 \  # <-- Replace with path to your downloaded .p8 key file
  --apple-key-id 66FGQZB566 \  # <-- Replace with your Apple Key ID from previous step
  --apple-issuer-id 0cd4db4a-5598-45b8-9d32-75cdf127d005 \  # <-- Replace with your Apple Issuer ID from previous step
  --certificate ./capgo-tutorial/capgo-build-tutorial-certificate.p12 \  # <-- Replace with path to your exported certificate
  --provisioning-profile ./capgo-tutorial/capgo_native_build_tutorial.mobileprovision \  # <-- Replace with path to your downloaded provisioning profile
  --apple-profile-name capgo_native_build_tutorial  # <-- Replace with the provisioning profile name you set previously
```
