---
title: Self-Hosting Capgo
description: A complete guide to self-hosting Capgo with MinIO (S3), edge functions, and the Capacitor updater plugin.
---
...
**Supabase (Postgres + Auth)** — Database, authentication, row-level security. Default ports: 5432 / 54321.

**Supabase Storage or MinIO** — Bundle file storage (S3-compatible). Default port: 9000.

**Edge Functions (Deno)** — Update, stats, and channel API endpoints. Default port: 54321.

**Capgo Web App (optional)** — Dashboard UI. Default port: 3000.
...
Capgo uses [Supabase](https://supabase.com/docs/guides/self-hosting) for its database and auth.
...
In your `capacitor.config.ts`, point the plugin at your self-hosted instance:
...
> **Tip:** If running MinIO and Supabase on a local network, use your machine's LAN IP rather than `localhost` — simulators and physical devices cannot reach `localhost` on your host machine.
...
Check Supabase logs on your device or simulator:
...
**Bundle downloads fail / 403 errors from MinIO**

Make sure the `capgo` bucket policy allows anonymous downloads:
...
Check that all environment variables in `.env` are set and that Supabase can reach MinIO on the `S3_ENDPOINT` you configured. Run `supabase functions serve --debug` for verbose logs.

**Device never receives updates**

Verify `updateUrl` in `capacitor.config.ts` uses your server's publicly reachable address (not `localhost`). Also confirm `notifyAppReady()` is called on every launch.

**Emails not sending**

Double-check your SMTP credentials in `supabase/config.toml` and ensure port 587 is open on your mail server.
...
For further configuration see the [Plugin Configuration reference](/docs/plugins/updater/settings/) and the [Self-Hosting auto-update guide](/docs/plugins/updater/self-hosted/auto-update/).
