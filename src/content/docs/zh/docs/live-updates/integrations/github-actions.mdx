...
locale: zh
description: "了解如何将 Capgo Live Updates 与 GitHub Actions 集成,实现应用更新的自动化部署。"
sidebar:
  order: 3
---
...
在您的 GitHub 存储库中设置必要的 secrets:

<Steps>

1. 导航到您的 GitHub 存储库
2. 进入 **Settings** → **Secrets and variables** → **Actions**
3. 点击 **New repository secret** 并添加以下内容:
...
从这个基本配置开始,在每次推送到主分支时部署到生产环境:

```yaml
# 用于 Capgo Live Updates 的简单 GitHub Actions 工作流
      ...
      - name: Install, test and build
        run: |
         ...
         run: |
           ...
           # 对于加密上传,添加: --key-data-v2 "${{ secrets.CAPGO_PRIVATE_KEY }}"
```
...
在本地[设置加密密钥](/docs/live-updates/encryption/#setting-up-encryption)后,将您的私钥添加到 GitHub secrets:
...
将此内容作为 `CAPGO_PRIVATE_KEY` 添加到您的 GitHub 存储库 secrets,然后在工作流中使用它:
...
**使用渠道进行测试**:部署到 PR 或开发渠道后,您可以通过配置应用使用该特定渠道来测试更新。了解更多关于[在应用中配置渠道](/docs/live-updates/channels/#configuring-the-channel-in-your-app)的信息。
...
- name: Send email notification
  ...
  with:
    ...
    subject: 'Capgo 部署失败 - ${{ github.repository }}'
    to: team@yourcompany.com
    from: ci-cd@yourcompany.com
    body: |
      ${{ github.repository }} 的部署失败
      分支: ${{ github.ref_name }}
      提交: ${{ github.sha }}
      工作流: ${{ github.workflow }}
```
...
**工作流失败并显示 "CAPGO_TOKEN not found":**
...
通过 GitHub Actions 集成,您可以利用 GitHub 强大的 CI/CD 平台为 Capgo Live Updates 创建复杂的部署工作流,并内置安全性、监控和协作功能。
