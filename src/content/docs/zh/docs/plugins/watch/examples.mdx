---
title: 示例与用例
description: 使用 Capacitor 构建 Apple Watch 应用的真实示例与模式。
sidebar:
  order: 4
locale: zh
---
...
本指南提供使用 `@capgo/capacitor-watch` 的常见 Apple Watch 应用模式示例。
...
| 方法 | 使用场景 | 交付方式 |
|--------|----------|----------|
| `sendMessage` | 交互、时效性强 | 立即(不可达时失败) |
| `updateApplicationContext` | 当前应用状态 | 仅最新值 |
| `transferUserInfo` | 重要数据 | 队列保证送达 |
...
- [Watch App Setup](/docs/plugins/watch/watch-app-setup/) - 创建您的第一个 watch 应用
- [API Reference](/docs/plugins/watch/getting-started/#api-reference) - 完整 API 文档
- [GitHub Repository](https://github.com/Cap-go/capacitor-watch) - 源码与问题反馈
