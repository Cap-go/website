---
title: Distribuisci un Live Update
sidebar:
  order: 3
locale: it
---
...
2 Configurato l'ID della tua app e il canale di aggiornamento in `capacitorconfig.ts`

3 Aggiunto nel tuo codice il metodo `CapacitorUpdater.notifyAppReady()`
...
2 Carica il bundle su Capgo:
   ...
   Questo caricherà una nuova versione del bundle sul canale specificato nel comando

### Risoluzione dei problemi di caricamento

Se il caricamento fallisce, verifica:
- Che l'ID della tua app in `capacitorconfig.ts` corrisponda alla tua app nella dashboard di Capgo
...
Se hai ancora problemi, vai alla sezione [Risoluzione dei problemi](/docs/getting-started/troubleshooting/)
...
3 Esegui la tua app localmente:
   ...
   npx cap run ios
   ```
   Oppure apri il progetto iOS/Android in Xcode/Android Studio ed esegui in modalità nativa

4 Mantieni l'app aperta per circa 30 secondi per permettere il download dell'aggiornamento in background
...
Fai riferimento al [Quickstart di Capgo](/docs/getting-started/quickstart#receiving-a-live-update-on-a-device) per maggiori dettagli sul test degli aggiornamenti live
