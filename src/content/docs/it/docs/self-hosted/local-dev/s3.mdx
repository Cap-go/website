...
description: Come configurare minio per funzionare con capgo
sidebar:
  order: 2
locale: it
---
...
Utilizzalo solo nello sviluppo locale
:::
...
Ora che abbiamo un server S3 in esecuzione dobbiamo configurare le funzioni edge di capgo per utilizzare il nostro server S3
...
PLAN_SOLO=test
PLAN_TEAM=test
# Di seguito la configurazione effettivamente importante per S3
...
Per eseguire le funzioni edge con il nostro nuovo file env usa
```bash
