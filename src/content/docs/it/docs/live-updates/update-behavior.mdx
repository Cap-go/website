...
Quando rilasci un aggiornamento alla tua app Capgo, probabilmente vuoi che i tuoi utenti lo ricevano il prima possibile. Ma non vuoi nemmeno interrompere la loro esperienza costringendoli ad attendere un download o riavviare l'app nel bel mezzo di una sessione.
...
Per impostazione predefinita, ecco come Capgo gestisce gli aggiornamenti delle app:
...
4. Quando l'utente avvierà nuovamente l'app, utilizzerà la versione aggiornata

</Steps>

Questo flusso garantisce che gli utenti utilizzino sempre l'ultima versione della tua app, senza mai essere interrotti da richieste di aggiornamento o costretti ad attendere i download.

<Aside type="tip">
Capgo controlla anche gli aggiornamenti quando l'app riprende dallo sfondo, quindi gli utenti riceveranno gli aggiornamenti anche se non chiudono completamente l'app
</Aside>

## Perché Questo Approccio?

Applicare gli aggiornamenti durante un evento di background o chiusura ha diversi vantaggi chiave per l'esperienza utente:

- Gli utenti non vengono interrotti da richieste di aggiornamento o costretti ad attendere i download durante una sessione

- Gli aggiornamenti vengono applicati senza interruzioni tra le sessioni, quindi l'esperienza di avvio dell'app è sempre aggiornata
...
Per mitigare questo, consigliamo di:

- Salvare lo stato frequentemente e ripristinarlo con grazia quando l'app riprende
...
- `nativeVersion`: Attendi l'installazione di un binario nativo con una versione minima prima di applicare l'aggiornamento
...
<Aside type="danger">
Nota che la condizione `kill` attualmente attiva l'aggiornamento dopo il primo evento di chiusura, non il prossimo evento di background come le altre condizioni. Questa inconsistenza sarà corretta in una futura release
</Aside>

## Applicazione Immediata degli Aggiornamenti

Per aggiornamenti critici o app con uno stato molto semplice, potresti voler applicare un aggiornamento non appena viene scaricato, senza attendere un evento di background o chiusura. Capgo supporta questo tramite l'opzione di configurazione `directUpdate`.
...
L'evento `appReady` si attiva una volta che l'app ha finito di inizializzarsi e di applicare eventuali aggiornamenti in sospeso. Questo è il momento in cui è sicuro mostrare l'interfaccia utente della tua app, poiché garantisce che l'utente vedrà l'ultima versione.

Oltre a gestire l'evento `appReady`, consigliamo di impostare l'opzione di configurazione `keepUrlPathAfterReload` su `true` quando si usa `directUpdate`. Questo preserva il percorso URL corrente quando l'app viene ricaricata a causa di un aggiornamento, aiutando a mantenere la posizione dell'utente nell'app e riducendo il disorientamento.

Se non gestisci l'evento `appReady` e non imposti `keepUrlPathAfterReload` quando usi `directUpdate`, l'utente potrebbe vedere brevemente una versione obsoleta dell'app, essere riportato alla route iniziale o vedere uno sfarfallio mentre l'aggiornamento viene applicato.
...
- Se l'aggiornamento modifica lo stato o l'interfaccia utente dell'app, l'utente potrebbe vedere un cambiamento dirompente nel mezzo di una sessione
...
Se abiliti `directUpdate`, consigliamo di:
