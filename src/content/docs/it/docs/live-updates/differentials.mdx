...
Il sistema di aggiornamento Live di Capgo può fornire aggiornamenti più velocemente ed efficientemente inviando solo i file modificati, invece dell'intero bundle JS
...
Un secondo vantaggio si ha quando l'app ha risorse di grandi dimensioni che cambiano raramente, come immagini o video, rispetto ai file JS compressi verranno scaricati solo una volta
...
Il plugin scarica quindi solo i file modificati, invece dell'intero bundle JS. Ricostruisce la nuova versione dell'app combinando questi file scaricati con i file non modificati che ha già
...
Quando viene usato `--partial-only`, Capgo caricherà solo i singoli file e genererà un manifest. Qualsiasi dispositivo che non supporta il parziale non sarà in grado di scaricare l'aggiornamento

Potresti voler usare `--partial-only` se:

- Vuoi sempre usare aggiornamenti differenziali e non vuoi mai permettere caricamenti del bundle completo
- Stai configurando una pipeline CI/CD e vuoi assicurarti che tutti i caricamenti automatizzati siano differenziali
- La tua app è grande e la larghezza di banda è limitata, quindi devi minimizzare le dimensioni di caricamento/download
...
Se gli aggiornamenti differenziali non sembrano funzionare (cioè i dispositivi stanno sempre scaricando il bundle JS completo anche per piccole modifiche), verifica che:
...
Puoi anche usare la webapp di Capgo per controllare i dettagli del tuo ultimo caricamento:
<Steps>
...
Se continui ad avere problemi, contatta il supporto Capgo per ulteriore assistenza. Possono controllare i log del server per confermare che i tuoi caricamenti parziali vengano elaborati correttamente e che i dispositivi stiano ricevendo i manifest aggiornati

Questo è tutto! Il flag `--partial` dice a Capgo di eseguire i caricamenti dei singoli file e la generazione del manifest necessari per gli aggiornamenti differenziali

Nota che devi usare `--partial` ogni volta che carichi una nuova versione che vuoi sia consegnata come aggiornamento differenziale. Se ometti il flag, Capgo caricherà l'intero bundle JS come un singolo file, e i dispositivi scaricheranno l'intero bundle anche se solo una piccola parte è cambiata