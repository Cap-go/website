---
title: Endpoint
description: Documentazione API pubblica di Capgo cloud
sidebar:
  order: 2
locale: it
---

Questa è la documentazione delle API pubbliche di Capgo cloud

Per accedere, aggiungi negli headers la tua chiave API come `authorization`
...
Recupera le informazioni dell'organizzazione. Se `orgId` è fornito nei parametri, restituisce una singola organizzazione. Altrimenti, restituisce tutte le organizzazioni accessibili
...
Questo endpoint ti permette di gestire le chiavi API per accedere all'API di Capgo
...
Crea una nuova chiave API per una specifica organizzazione
...
Recupera le statistiche per una specifica app
...
Recupera statistiche aggregate di tutte le organizzazioni a cui l'utente ha accesso
...
e opzionalmente nome `channel` come parametro URL per riceverne uno:
...
Invia `app_id` come parametro URL e ricevi array dei primi 50 dispositivi\
È possibile ottenere il successivo inviando `page=1`
...
e opzionalmente `device_id` come parametro URL per riceverne uno:
