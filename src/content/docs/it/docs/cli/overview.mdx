...
Utilizza la funzione Live Updates di Capgo per aggiornare i bundle JavaScript della tua app da remoto, in tempo reale. Invia aggiornamenti JS direttamente ai tuoi utenti senza passare attraverso il processo di revisione dell'app store per correggere istantaneamente bug e distribuire nuove funzionalità.

<Aside>

Gli aggiornamenti live sono limitati alle modifiche del bundle JavaScript. Se devi aggiornare il codice nativo, come aggiungere o rimuovere un plugin o modificare la configurazione del progetto nativo, dovrai inviare una nuova build binaria nativa agli app store.
...
- Inserire i tuoi processi, come la firma del codice, il testing QA o le approvazioni dei manager, tra le fasi di build e deploy
...
Questo comando verrà poi salvato per uso futuro. Non dovrai fornire la tua chiave API con ogni comando dopo aver effettuato l'accesso.

### Differenze principali rispetto ad altri strumenti CLI
...
- Capgo utilizza una singola CLI sia per lo sviluppo che per i casi d'uso CI/CD, poiché Capgo è focalizzato esclusivamente sulle funzionalità di aggiornamento live

- La CLI di Capgo non richiede un passaggio di installazione separato. È inclusa nel pacchetto `@capgo/cli` e può essere eseguita direttamente usando `npx`

- La CLI di Capgo è progettata specificamente per il flusso di lavoro degli aggiornamenti live, quindi potrebbe non includere alcune funzionalità o comandi presenti in strumenti CLI più generici
