...
**Importante**: Desde la versión del plugin **v6.25.0** y **v7.25.0**, el ID del dispositivo ahora persiste entre reinstalaciones de la aplicación. Antes de estas versiones, cada reinstalación de la aplicación generaba un nuevo ID de dispositivo y contaba como un nuevo MAU.

Con las versiones actuales:
- El ID del dispositivo persiste entre reinstalaciones de la aplicación (almacenado de forma segura en Keychain en iOS y EncryptedSharedPreferences en Android)
- Actualizar la aplicación no crea un nuevo ID de Dispositivo
- Durante el desarrollo, si estás usando una versión anterior del plugin (< v6.25.0 / v7.25.0), cada reinstalación todavía crea un nuevo MAU

Nota: Las descargas de TestFlight y los cambios de canal en Android pueden seguir generando nuevos registros de dispositivos dependiendo de tu configuración.
...
Puedes actualizar o bajar de categoría tu plan en cualquier momento en tu panel de control: [https://console.capgo.app/settings/organization/plans](https://console.capgo.app/settings/organization/plans/)
...
**Persistencia del ID de Dispositivo (Actualizado en v6.25.0 y v7.25.0)**:
- **Comportamiento actual**: El ID del dispositivo ahora persiste entre reinstalaciones de la aplicación. Se almacena de forma segura en el Keychain del dispositivo (iOS) o EncryptedSharedPreferences (Android), permitiéndonos rastrear el mismo dispositivo incluso después de una desinstalación/reinstalación.
- **Comportamiento anterior** (antes de v6.25.0/v7.25.0): Por razones de privacidad relacionadas con las políticas de la App Store de Apple y Google, el ID del dispositivo se reiniciaba en cada reinstalación de la aplicación, haciendo imposible rastrear el mismo dispositivo entre reinstalaciones.

Las reglas de privacidad son impuestas por Apple y Google, y la implementación de Capgo cumple con sus mejores prácticas para la identificación de dispositivos.
