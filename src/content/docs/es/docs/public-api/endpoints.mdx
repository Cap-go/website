...
Para acceder, agrega en los encabezados tu clave API como `authorization`
...
Requiere un cuerpo de solicitud de tipo `OrganizationUpdate`

Al tener éxito, devuelve lo siguiente:
...
Requiere un cuerpo de solicitud de tipo `OrganizationCreate`
...
Requiere un cuerpo de solicitud (parámetros de consulta) de tipo `{ orgId: string }`
...
Requiere un cuerpo de solicitud (parámetros de consulta) de tipo `MemberDelete`
...
Si se proporciona el parámetro `graph`, devuelve una imagen SVG con la visualización de la métrica solicitada
...
Envía `app_id` como parámetro URL y recibe un array de los primeros 50 canales\
...
Envía `app_id` como parámetro URL y recibe un array de los primeros 50 dispositivos\
...
y opcionalmente el nombre `device_id` como parámetro URL para recibir uno:
...
Este endpoint es para desvincular el canal y anular la versión. No puedes eliminar un dispositivo de Capgo
