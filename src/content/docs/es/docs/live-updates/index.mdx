...
description: "Descubre cómo las Actualizaciones en Vivo de Capgo permiten actualizaciones fluidas de bundles JavaScript, permitiéndote enviar cambios directamente a los usuarios sin demoras de la tienda de aplicaciones."
sidebar:
  order: 1
prev: false
next: false
...
Usa la función de Actualizaciones en Vivo de Capgo para actualizar los bundles JavaScript de tu aplicación de forma remota y en tiempo real. Envía actualizaciones JS directamente a tus usuarios sin pasar por el proceso de revisión de la tienda de aplicaciones para corregir errores al instante y lanzar nuevas funciones.

<Aside>

Las Actualizaciones en Vivo están limitadas a cambios en el bundle JavaScript. Si necesitas actualizar código nativo, como agregar o eliminar un plugin o cambiar la configuración del proyecto nativo, necesitarás enviar una nueva compilación binaria nativa a las tiendas de aplicaciones.
...
Cuando subes un nuevo bundle JS a Capgo y lo asignas a un canal, el SDK de Capgo en aplicaciones configuradas para ese canal detectará la actualización y la descargará. La próxima vez que la aplicación se reinicie, se cargará el nuevo bundle.
...
3. Compila tu bundle JS y súbelo a Capgo:
   ```shell
   npm run build
...
5. Una vez que se descarga la actualización, cierra y vuelve a abrir tu aplicación para cargar el nuevo bundle.
...
<CardGrid stagger>
  ...
  <a href="/docs/live-updates/rollbacks/">
    <Card title="Reversiones" icon="left-arrow">
      Descubre cómo revertir a una versión anterior del bundle JS si una actualización causa problemas.
    </Card>
  </a>
