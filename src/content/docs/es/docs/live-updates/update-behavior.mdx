...
Cuando publicas una actualización de tu aplicación Capgo, probablemente quieras que tus usuarios reciban esa actualización lo antes posible. Pero tampoco quieres interrumpir su experiencia forzándolos a esperar una descarga o reiniciar la aplicación en medio de una sesión.

El comportamiento de actualización de Capgo está diseñado para encontrar un equilibrio entre entregar actualizaciones rápidamente y minimizar la interrupción para tus usuarios.
...
4. Cuando el usuario vuelva a iniciar la aplicación, ejecutará la versión actualizada

</Steps>

Este flujo asegura que los usuarios siempre ejecuten la última versión de tu aplicación, sin ser interrumpidos por avisos de actualización ni verse obligados a esperar descargas.
...
Aplicar actualizaciones en segundo plano o al cerrar tiene varios beneficios clave para la experiencia del usuario:
...
La principal desventaja es que si un usuario envía a segundo plano y rápidamente reanuda tu aplicación, pueden perder cualquier estado no guardado ya que la actualización se aplicó entre esas acciones.
...
- Considerar personalizar el comportamiento de actualización para flujos sensibles (ver más abajo)
...
Este ejemplo retrasaría la instalación de una actualización hasta después del 1 de junio de 2023 Y que la aplicación haya estado en segundo plano durante al menos 60 segundos.
...
<Aside type="danger">
Ten en cuenta que la condición `kill` actualmente activa la actualización después del primer evento de cierre, no del próximo evento en segundo plano como las otras condiciones. Esta inconsistencia se corregirá en una versión futura
</Aside>
...
Con `directUpdate` habilitado, Capgo aplicará inmediatamente una actualización tan pronto como se complete la descarga, incluso si el usuario está usando activamente la aplicación.
...
Si no manejas el evento `appReady` y estableces `keepUrlPathAfterReload` cuando uses `directUpdate`, el usuario podría ver brevemente una versión desactualizada de la aplicación, ser llevado de vuelta a la ruta inicial, o ver un parpadeo mientras se aplica la actualización.

Usar `directUpdate` puede ser útil para entregar correcciones críticas de errores o parches de seguridad, pero viene con algunas compensaciones:
...
- La ubicación del usuario en la aplicación puede perderse si no se establece `keepUrlPathAfterReload`, potencialmente desorientándolo
