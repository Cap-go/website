...
**Lo que el cifrado realmente protege**: A diferencia de los sistemas OTA que solo firman las actualizaciones, Capgo cifra el bundle cargado antes del almacenamiento y la entrega. Esto protege el contenido del bundle frente a accesos casuales en almacenamiento y tránsito y garantiza que solo alguien con su clave privada pueda producir una actualización cifrada válida. **No** hace imposible realizar ingeniería inversa sobre los activos web distribuidos: la clave pública que el cliente usa para descifrar las actualizaciones se distribuye dentro de la app, por lo que un atacante decidido aún puede extraerla e inspeccionar el contenido del bundle con suficiente esfuerzo.

<Aside type="note" title="Cuándo necesita cifrado">
Si sube un bundle sin cifrado, trátelo como un activo público. Los canales privados limitan qué dispositivos reciben una actualización, pero no hacen confidencial el bundle cargado. El cifrado es útil cuando desea una mejor protección en almacenamiento y tránsito y cuando quiere que solo quienes poseen la clave privada puedan publicar actualizaciones cifradas válidas. No garantiza que el JavaScript, HTML o CSS distribuido nunca pueda ser inspeccionado.
</Aside>

<Aside type="caution" title="Modelo de amenazas">
El cifrado de Capgo protege contra la exposición del bundle por parte de Capgo, proveedores de almacenamiento, CDN o cualquier persona que solo vea el artefacto cifrado de entrega. También evita que terceros generen actualizaciones cifradas válidas sin su clave privada. Como la clave pública está integrada en la app distribuida para que el cliente pueda descifrar las actualizaciones, alguien que tenga el binario de su app y suficiente motivación todavía puede recuperar el material de clave necesario para inspeccionar el bundle.
</Aside>
...
|---------|-------|-------------------|
| **Contenido del Paquete** | Cifrado en almacenamiento y tránsito; aún puede ser inspeccionado por un atacante decidido con el binario de la app | Públicamente legible |
| **Método de Seguridad** | Cifrado verdadero de extremo a extremo | Solo firma de código |
| **Nivel de Privacidad** | Protección fuerte en entrega y almacenamiento; no evita la ingeniería inversa | La plataforma puede acceder a su código |
...
- **El cifrado de Capgo** protege el bundle mientras se almacena y se entrega, y dificulta mucho la falsificación de actualizaciones cifradas porque un atacante necesitaría su clave privada
- **La ingeniería inversa sigue siendo posible** después de distribuir la app, porque el cliente contiene la clave pública necesaria para descifrar y cargar la actualización
...
- **Clave Pública en Config**: Debe ejecutar `key save` para almacenar la clave pública en su configuración de Capacitor para que la aplicación móvil la Usar.
...
- Usar actualizaciones diferenciales para minimizar transferencia de datos cifrados
...
- Configurar [Integración CI/CD](/docs/getting-started/cicd-integration/) para automatizar despliegues cifrados
