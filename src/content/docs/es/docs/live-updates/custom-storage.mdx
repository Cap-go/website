...
description: "Aprenda cómo usar soluciones de almacenamiento personalizadas con Capgo Live Updates, incluyendo URLs externas, integración S3 y cifrado de paquetes para despliegues seguros."
sidebar:
  order: 6
locale: es
---
...
El comando zip devolverá el checksum del archivo zip. Puede usar este checksum para cifrar el archivo zip si es necesario. Use la opción `--json` para obtener salida estructurada incluyendo el checksum.
...
| `--package-json` | Rutas a archivos package.json para monorepos (separados por comas) |
...
El parámetro `CHECKSUM` es requerido y debe ser el checksum de su archivo zip. Puede obtener el checksum de la salida del comando zip (use la opción `--json` para salida estructurada).
...
- Use HTTPS para todas las URLs de paquetes (requerido para iOS y Android)
...
- Verifique que la URL sea públicamente accesible y use HTTPS (requerido para iOS y Android)
...
- Asegúrese de que se use el cifrado v2 para nuevos paquetes
...
- Configure [Integración CI/CD](/docs/getting-started/cicd-integration/) para automatizar su flujo de trabajo de almacenamiento personalizado
