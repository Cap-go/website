...
Si deseas gestionar por ti mismo cuándo se aplica la actualización, utiliza el modo manual con Capgo cloud

Esto es lo que necesitas hacer, configura tu cuenta como se explica en Comenzando
...
App.addListener('appStateChange', async (state: any) => {
  ...
  if (!state.isActive && data) {
    console.log('set')
    // Realiza el cambio cuando el usuario sale de la aplicación o cuando lo desees
    SplashScreen.show()
    try {
