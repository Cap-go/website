...
description: Problemas conocidos de Capacitor y CapGo
sidebar:
  order: 7
locale: es
---
...
* Cuando desarrollas, si utilizas la función de recarga en vivo de Ionic desde la CLI, anulará el plugin, por lo que nunca verás tus actualizaciones
...
* Esto suele ocurrir cuando se envían actualizaciones grandes (> 20mb), un gran porcentaje de usuarios no obtendrá la última versión
Anteriormente, los usuarios necesitaban mantener la aplicación abierta hasta que la descarga se completara, ahora utilizamos descarga en segundo plano, pero aún está limitada a unos pocos segundos

## Android

### No se puede descargar

Hemos visto algunos problemas con dispositivos en India, y al hablar con usuarios, les hicimos probar diferentes servidores DNS, y funcionó

Entonces, si tienes el problema, intenta usar un servidor DNS diferente como Cloudflare o Google DNS
...
### Auto-alojado
Cuando estás implementando una actualización auto-alojada, ten en cuenta que no puedes usar endpoints "HTTP" ya que va en contra de las políticas de seguridad de las aplicaciones Android, si aún quieres hacerlo, sigue esta guía:
...
Recomendamos declarar [`CA921`](https://developerapplecom/documentation/bundleresources/privacy_manifest_files/describing_use_of_required_reason_api#4278401) como la razón para acceder a la API [`UserDefaults`](https://developerapplecom/documentation/foundation/userdefaults)

### Permisos de red

Cuando uses un servidor local para probar actualizaciones, la aplicación pedirá permisos de red, es un comportamiento normal, no ocurre cuando usas un servidor remoto
