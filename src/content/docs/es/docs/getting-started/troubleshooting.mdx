...
Si tu carga de bundle falla, verifica dos veces:
...
- `--tus`: Usa el [protocolo de carga reanudable tus](https://tus.io/) para cargas más confiables de bundles grandes o en conexiones de red deficientes. Si tu bundle es mayor a 10MB o estás en una conexión irregular, considera usar `--tus`:
  ```shell
...
2. Lanza tu aplicación en un dispositivo o emulador y realiza la acción que debería activar una actualización (por ejemplo, reabrir la aplicación después de subir un nuevo bundle).
...
4. Usa los registros de depuración para identificar dónde está ocurriendo el problema. Por ejemplo:
   - Si no se encuentra ninguna actualización, verifica dos veces que tu bundle se subió exitosamente y que la aplicación está configurada para usar el canal correcto.
...
- Falta de espacio de almacenamiento para el bundle de actualización
