...
> ⚡️ Capgo automatiza canales, limpieza de bundles, reversiones, analíticas y cargas CLI de forma nativa. Usa esta guía para realizar los pasos mínimos requeridos para migrar y opcionalmente recrear cualquier comportamiento personalizado que aún necesites.

## Descripción general

1. Recopila tu configuración existente de Capawesome Cloud (App ID, canales, claves de firma, tokens CLI) para que puedas archivarla o auditarla más tarde.
2. Instala el plugin de Capgo, elimina el SDK de Capawesome y llama a `CapacitorUpdater.notifyAppReady()`.
3. Configura comportamientos opcionales (descargas manuales, fijación de bundles, recargas) si dependes de esos flujos actualmente.

Con Capgo solo necesitas instalar nuestro plugin y llamar a `CapacitorUpdater.notifyAppReady()`. Todo lo demás—canales, limpieza de bundles, reversiones, analíticas y automatización CLI—se maneja de forma nativa. Las secciones a continuación explican cada tarea directamente.

## Antes de comenzar

- Asegúrate de que tu proyecto ya esté usando Capacitor 5 o posterior.
- Instala el CLI de Capgo (`npm install -g @capgo/cli`) si planeas enviar bundles desde CI/CD.
...
| `maxVersions` / retención | Política de retención de bundles | Configurado centralmente en Capgo (1 mes por defecto, 24 meses máx.) |
...
Esto confirma que la app se inició correctamente. Si la confirmación nunca llega, Capgo revierte automáticamente el bundle—no se necesita JavaScript adicional.
...
| `LiveUpdate.setNextBundle()` | `CapacitorUpdater.next()` | Opcional: el panel fija bundles automáticamente |
| `LiveUpdate.reload()` | `CapacitorUpdater.reload()` | Opcional; Capgo aplica bundles obligatorios después de `notifyAppReady()` |
...
**Establecer el siguiente bundle**
...
**Aplicar el bundle descargado inmediatamente**
...
- **Capgo**: marca el bundle como "obligatorio" en el panel, luego escucha el evento `majorAvailable` (emitido después de `notifyAppReady()`) para requerir que los usuarios actualicen dentro de tu app.

## Paso 6 – Desplegar bundles
...
Dado que Capgo rastrea la salud de los bundles automáticamente, también obtienes:
...
Capgo está construido para confiabilidad a largo plazo: actualizaciones delta nativas, bundles encriptados, reversiones automáticas y analíticas que no requieren JavaScript personalizado. Una vez que migres, puedes eliminar el código de mantenimiento pesado y dejar que la plataforma ejecute actualizaciones automáticamente.
