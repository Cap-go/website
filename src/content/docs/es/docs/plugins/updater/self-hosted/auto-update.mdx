...
## Servir tu bundle
Asegúrate de que tu bundle se sirva a través de HTTPS, y que el servidor tenga los encabezados CORS correctos para permitir que la aplicación descargue la actualización.
...
```bash title="Crear bundle con información de checksum"
...
Usa el valor `checksum` de esta salida en la respuesta de tu API para asegurar que el plugin pueda verificar la integridad del bundle antes de la instalación.
