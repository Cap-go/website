...
<Aside type="caution" title="公開アセットモデル">
Capgoにアップロードされる未暗号化のバンドルは、公開の配信アーティファクトとして扱う必要があります。チャンネルはどのデバイスにアップデートを提示するかを決めますが、チャンネルのプライバシー設定によって基になるバンドルが機密になるわけではありません。暗号化は保存時と転送時の保護を追加し、第三者が有効な暗号化アップデートを生成することを防ぎますが、復号に必要な公開鍵がアプリに含まれているため、配信済みのWebアセットを不可能にするわけではありません。[エンドツーエンド暗号化](/docs/live-updates/encryption/)で正確な脅威モデルを確認してください。
</Aside>
...
<CardGrid stagger>
  <a href="/docs/live-updates/update-types/">
    <Card title="アップデートタイプ" icon="open-book">
      すべてのOTAアップデートタイプのリファレンス：適用タイミング、遅延条件、バージョンブロッキング、配信
    </Card>
  </a>
