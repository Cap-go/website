...
description: Capacitor Updater を自己ホスト型 Capgo で使用する方法
sidebar:
  order: 2
locale: ja
---
...
このチュートリアルでは、セルフホスト型のcapgoを使用して開発環境でcapacitor updaterを使用する方法を説明します
...
セルフホスト型capgoでcapacitor updaterを使用するには、アプリディレクトリの`capacitorconfigts`を編集して次のように設定します:
...
AndroidではデフォルトでローカルのSSLが無効になっているため、[プレーンテキスト通信](https://developerandroidcom/topic/security/risks/cleartext/)を有効にする必要があります
...
Androidアプリの接続を妨げる問題が発生する可能性もあります。エッジ関数へのリクエストが送信されているのが確認できない場合は、以下を実行してください:
```bash
