---
title: Self-hosting di S3
description: mingioをcapgoで動作するように設定する方法
sidebar:
  order: 2
locale: ja
---
...
これは技術的にはcapgoを実行するために必須ではありません。
...
このコンテナのコンソールウィンドウを閉じた場合は、以下のコマンドで起動できます：
```bash
docker start minio1
```
...
S3サーバーが動作したので、S3サーバーを使用するようにcapgoエッジ関数を設定する必要があります。
...
`hostdockerinternal`はdocker内部からのみアクセス可能なdocker IPですが、コード内で`0000`に置き換えることでlocalhost経由でminioにアクセスできます。

新しいenv fileでエッジ関数を実行するには以下を使用します：
```bash
...
CLIはデフォルトではminioでは動作しません。`capacitorconfig.ts`[^1]に以下の変更が必要です：
```ts
