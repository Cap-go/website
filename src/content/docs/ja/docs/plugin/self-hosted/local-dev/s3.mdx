...
description: minioをcapgoで動作するように設定する方法
sidebar:
  order: 2
locale: ja
---
...
S3サーバーが動作したら、capgoエッジ関数がS3サーバーを使用できるように設定する必要があります
...
IPアドレス`hostdockerinternal`はdocker内部からのみアクセス可能なdocker IPですが、minioにlocalhostからアクセスできるようにコード内で`0000`に置き換えられます

新しい環境ファイルでエッジ関数を実行するには以下を使用します
```bash
