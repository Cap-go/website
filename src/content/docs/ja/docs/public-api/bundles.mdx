---
title: "Bundles"
description: "アプリ更新パッケージのリスト表示、削除、バージョン管理、ストレージ最適化をカバーする、Capgo Bundles管理の詳細ガイド。"
sidebar:
  order: 8
---

Bundlesは、Capgoのコア更新パッケージです。各バンドルには、アプリのコンテンツを構成するWebアセット（HTML、CSS、JS）が含まれています。Bundles APIを使用すると、これらの更新パッケージを管理でき、リスト表示や削除が可能です。

## Bundlesを理解する

バンドルは、アプリのWebコンテンツの特定バージョンを表し、以下を含みます：
- **Version**：バンドルのセマンティックバージョン番号
- **Checksum**：バンドルの整合性を検証するための一意のハッシュ
- **Storage Info**：バンドルがどこにどのように保存されているかの詳細
- **Native Requirements**：ネイティブアプリの最小バージョン要件
- **Metadata**：作成時刻、所有権、その他の追跡情報

## 手動によるBundle作成（CLIなし）

Capgo CLIを使用せずに手動でバンドルを作成およびアップロードする方法：

### ステップ1：アプリをビルドする

まず、アプリのWebアセットをビルドします：

```bash
npm run build
```

### ステップ2：Capgo CLIと同じパッケージを使用してBundleのZipを作成する

**重要**：互換性を確保するために、Capgo CLIが内部で使用しているのとまったく同じJavaScriptパッケージを使用してください。
...
#### JavaScriptでZip Bundleを作成する（Capgo CLIと同じ）
...
### ステップ3：CLIと同じパッケージを使用してSHA256チェックサムを計算する
