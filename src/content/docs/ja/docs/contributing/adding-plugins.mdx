...
| `/src/content/plugins/en/` | 英語チュートリアルファイル |
...
7. ### チュートリアルファイルを作成
   ...
   `/src/content/plugins/en/your-plugin-name.md`を作成:
...
- [ ] `/src/content/plugins/en/`にチュートリアルを作成
- [ ] インストール手順を含める
- [ ] iOS設定をドキュメント化
- [ ] Android設定をドキュメント化
- [ ] 使用例を提供
...
2. 生成された翻訳を確認:
   - `/src/content/plugins/de/`（ドイツ語）
   - `/src/content/plugins/es/`（スペイン語）
   - `/src/content/plugins/fr/`（フランス語）
   - `/src/content/plugins/it/`（イタリア語）
   - `/src/content/plugins/ja/`（日本語）
   - `/src/content/plugins/ko/`（韓国語）
   - `/src/content/plugins/id/`（インドネシア語）
