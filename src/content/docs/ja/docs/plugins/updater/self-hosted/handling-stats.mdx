...
description: >-
  How to use the statistics endpoint of Capgo with the auto-update plugin in
...
更新エンドポイントに新しいバージョンがない場合は、次のようなエラーペイロードで応答する必要があります:

```json
{ "error": "no_new_version_available", "message": "No new version available" }
```

`error` コードは必ず `no_new_version_available` にしてください。`message` は任意の文字列で構いません（ログ/デバッグ用）。これは期待される挙動で、HTTP `200` で返します。`url` なしの `200` を返すと、プラグインはダウンロード失敗として扱い `download_fail` を送信します。
