---
title: >-
  Migration Guide from @capacitor-community/facebook-login to
  @capgo/capacitor-social-login
description: >-
  This detailed guide provides step-by-step instructions for transitioning from
  the @capacitor-community/facebook-login plugin to the
  @capgo/capacitor-social-login plugin, ensuring a smooth migration process with
  enhanced features.
sidebar:
  order: 2
locale: ja
...
このガイドは、`@capacitor-community/facebook-login` から `@capgo/capacitor-social-login` への移行に関する包括的な指示を提供します。新しいプラグインは、複数のソーシャル プロバイダーをサポートする統一された API、改善された TypeScript サポート、および拡張機能を備えた Facebook 認証を最新化します。
