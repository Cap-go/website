...
このガイドは、レガシー `@capacitor-community/apple-sign-in` プラグインから最新の `@capgo/capacitor-social-login` パッケージへの移行を説明しています。新しいプラグインは、複数のソーシャル認証プロバイダー向けの統一されたインターフェースを提供し、TypeScript サポートの改善とアクティブなメンテナンスが特徴です。
