...
description: >-
  Step-by-step guide to creating auto-renewable subscriptions in App Store
  Connect for the native-purchases plugin.
sidebar:
  order: 8
locale: ja
---
...
自動更新サブスクリプションは、iOS アプリ内のコンテンツ、サービス、またはプレミアム機能への継続的なアクセスを提供します。このガイドでは、App Store Connect でサブスクリプションを作成する方法を説明します。
