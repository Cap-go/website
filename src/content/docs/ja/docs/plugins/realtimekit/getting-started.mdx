...
description: >-
  Learn how to integrate Cloudflare Calls video conferencing into your Capacitor
  app
locale: ja
---

このガイドでは、Cloudflare Callsを活用したビデオ会議を追加するために、Capacitor RealtimeKitプラグインを統合する方法を説明します。
...
1. **早期に初期化**: アプリの起動時にRealtimeKitを初期化します
2. **エラー処理**: 常にtry-catchブロックで呼び出しをラップします
3. **権限をリクエスト**: 開始前にカメラ/マイク権限を確認します
4. **ネットワークをテスト**: 参加前にインターネット接続を確認します
5. **バックグラウンドオーディオ**: iOSのバックグラウンドモードを設定します
6. **ユーザー体験**: 初期化中に読み込み状態を表示します
7. **URLをクリーンにする**: 使用前にミーティングURLを検証します
...
- See the [tutorial](/plugins/capacitor-realtimekit) for complete implementation
