...
locale: fr
title: "Bundles"
description: "Detailed Guide for managing Capgo Bundles, covering listing, deleting, versioning, and storage optimization for Application Mise à jour packages."
sidebar: 
  order: 8
---

Bundles are the core Mise à jour packages in Capgo. Each Bundle contains the web assets (HTML, CSS, JS) that make up your Application's content. The Bundles API allows you to manage these Mise à jour packages, including listing and deleting them.

## Understanding Bundles

A Bundle represents a specific Version of your Application's web content and includes:
- **Version**: Semantic Version number of the Bundle
- **Checksum**: Unique hash to verify Bundle integrity
- **Storage Info**: Details À propos where and how the Bundle is stored
- **Natif Requirements**: Minimum Natif Application Version requirements
- **Metadata**: Creation time, ownership, and other tracking Information

## Manual Bundle Creation (Without CLI)

Here's how to Créer and Télécharger Bundles manually without using the Capgo CLI:

### Step 1: Construction Your Application

First, Construction your Application's web assets:

```bash
npm run build
```

### Step 2: Créer Bundle Zip Using Same Packages as Capgo CLI
...
#### Créer Zip Bundle with JavaScript (Same as Capgo CLI)
...
### Step 4: Télécharger Bundle to Your Storage

Télécharger your zip file to any web-accessible storage:
...
## Terminé Manual Workflow Exemple

Simple Node.js script to zip, checksum, and Télécharger to Capgo:
...
- **Plugin Verification**: The mobile plugin verifies checksums before applying Mises à jour

## Best Practices

1. **Version Management**: Use semantic versioning consistently
2. **Storage Optimization**: Retirer unused Bundles periodically
3. **Version Compatibility**: Set appropriate minimum Natif Version requirements
...
Retrieve Bundle Information. Retourne 50 Bundles per page.
...
Supprimer one or all Bundles for an Application. Use with Attention as this action cannot be undone.
...
Créer a Nouveau Bundle with external URL.
...
Mise à jour Bundle metadata such as link and comment Information.
...
Set a Bundle to a specific Canal. This links a Bundle Version to a Canal for distribution.
...
2. **Size Management**: Monitor Bundle sizes and storage Utilisation
3. **Backup Strategy**: Consider backing up critical versions
4. **Cost Optimization**: Retirer unnecessary Bundles to optimize storage costs 
