...
description: Enregistrer and manage Construction credentials locally for iOS and Android builds
sidebar: 
  order: 5
---
...
If you don't have your certificates and credentials yet, Vérifier these comprehensive guides:

**iOS:**
- [How to Get iOS Certificates](/docs/CLI/cloud-Construction/iOS/#how-to-get-iOS-certificates-and-provisioning-profiles) - Step-by-step Guide
- [iOS Certificates Guide (Capawesome)](https://capawesome.io/cloud/Natif-builds/certificates/iOS/) - Detailed Tutoriel with screenshots
- [Blog: Automatic iOS Builds](https://capgo.Application/blog/automatic-capacitor-iOS-Construction-github-action/) - Terminé CI/CD Configuration

**Android:**
- [Creating a Keystore](/docs/CLI/cloud-Construction/Android/#creating-a-keystore) - Step-by-step Guide
- [Android Certificates Guide (Capawesome)](https://capawesome.io/cloud/Natif-builds/certificates/Android/) - Detailed Tutoriel with screenshots
- [Blog: Automatic Android Builds](https://capgo.Application/blog/automatic-capacitor-Android-Construction-github-action/) - Terminé CI/CD Configuration
</Aside>

<Aside type="caution" title="Security Guarantee">
**Your credentials are NEVER stored permanently on Capgo servers:**
- ✅ Used ONLY during the Actif Construction process
- ✅ Automatically deleted after Construction completion
- ✅ Maximum retention: 24 hours (even if Construction fails)
- ✅ Apps are sent directly to Application Store/Play Store - we store NOTHING
...
Store your Construction credentials locally for automatic use:
...
Retirer saved credentials from your local machine:
...
When you Enregistrer iOS credentials, the CLI:
1. Reads the certificate and provisioning Profil files
...
When you Enregistrer Android credentials, the CLI:
1. Reads the keystore and service Compte JSON files
2. Converts them to base64 encoding
3. Saves the credentials to the `.capgo-credentials` folder
...
Once you've saved credentials, they're automatically used when you Construction:
...
1. **iOS**: Generate Nouveau certificates and Clés API yearly
2. **Android**: Change keystore passwords annually
3. **After team changes**: Rotate when team Membres leave
...
If the Construction says no credentials were found:

1. **Vérifier if credentials are saved**:
   ```bash
   npx @capgo/cli build credentials list
   ```

2. **Enregistrer credentials if missing**:
   ```bash
...
Vérifier that the correct platform is specified:
...
### Clear and re-Enregistrer credentials
...
If you're currently using environment variables, you can Migrer to saved credentials:
...
2. **Decode base64 files Retour to original files** (if needed)
   ```bash
   echo "$BUILD_CERTIFICATE_BASE64" | base64 -d > cert.p12
   echo "$BUILD_PROVISION_PROFILE_BASE64" | base64 -d > profile.mobileprovision
   ```

3. **Enregistrer using the CLI**
   ```bash
   npx @capgo/cli build credentials save \
     --platform ios \
     --certificate ./cert.p12 \
...
4. **Test the Construction**
   ```bash
   npx @capgo/cli build com.example.app --platform ios
   ```

5. **Retirer environment variables** (optional)
   ```bash
...
- [Commencer](/docs/CLI/cloud-Construction/getting-started/) - Créer your first Construction
- [iOS Builds](/docs/CLI/cloud-Construction/iOS/) - iOS-specific Construction Configuration
- [Android Builds](/docs/CLI/cloud-Construction/Android/) - Android-specific Construction Configuration
