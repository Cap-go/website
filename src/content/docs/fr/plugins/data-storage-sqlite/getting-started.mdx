---
title: Commencer
description: Learn how to Installer and configure the Capacitor Data Storage SQLite plugin for fast key-value storage with optional Chiffrement.
sidebar: 
  order: 2
---
...
Retirer a key-value pair.

#### `clear()`
Clear all data from storage.

#### `iskey(options: IskeyOptions)`
Vérifier if a key exists.
...
### iOS
- Uses SQLite3 with optional SQLCipher for Chiffrement
- Data persists across Application Mises à jour
- Supports iOS 11.0+
...
1. **Utilisateur Preferences**: Store Application Paramètres and preferences
2. **Cache Management**: Cache API responses and data
3. **Offline Storage**: Store data for offline access
4. **Session Management**: Manage Utilisateur sessions securely
5. **Token Storage**: Securely store authentication tokens

## Best Practices

1. **Use Chiffrement for Sensitive Data**
   ```typescript
...
- Vérifier database name is valid (alphanumeric, underscores)
- Ensure no special characters in database name
- Verify Chiffrement mode matches existing database

**Data not persisting:**
- Ensure `openStore` is called before operations
- Vérifier for any errors in console
