---
title: Commencer
description: Learn how to Installer and use the Background Geolocation plugin to track Appareil location in the background.
sidebar: 
  order: 2
---
...
If your Application forwards location Mises à jour to a server in real time, be aware that after 5 minutes in the background Android will throttle HTTP requests initiated from the WebView. The Solution is to use a Natif HTTP plugin such as [CapacitorHttp](https://capacitorjs.com/docs/apis/http). See https://github.com/capacitor-Communauté/background-geolocation/issues/14.
...
To Démarrer listening for changes in the Appareil's location, call this method.
A Promise is returned to indicate that it finished the call. The callback will be called every time a Nouveau location
is Disponible, or if there was an Erreur when calling this method. Don't rely on promise rejection for this.
...
Opens the Appareil's location Paramètres page.
Useful for directing Utilisateurs to Activer location services or adjust permissions.
...
Plays a sound file when the Utilisateur deviates from the planned route.
This should be used to play a sound (in the background too, only for Natif).
...
The Options for configuring for location Mises à jour.
...
Erreur object that may be passed to the location Démarrer callback.
Extends the standard Erreur with optional Erreur codes.
