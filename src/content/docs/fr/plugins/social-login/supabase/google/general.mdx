---
title: Supabase Google Connexion - General Configuration
description: Learn how to set up Google Sign-In with Supabase Authentication using the Capacitor Social Connexion plugin.
sidebar: 
    order: 1
---
...
This Guide will walk you through integrating Google Sign-In with Supabase Authentication using the Capacitor Social Connexion plugin. This Configuration allows you to use Natif Google Sign-In on mobile platforms while leveraging Supabase Auth for backend authentication.
...
2. Read the [Google Connexion General Configuration](/docs/plugins/social-Connexion/google/general/) Guide to Configuration Google OAuth credentials
3. Followed the respective platform-specific guides to Configuration Google OAuth credentials for your target platform:
    - [Android Configuration](/docs/plugins/social-Connexion/google/Android/)
    - [iOS Configuration](/docs/plugins/social-Connexion/google/iOS/)
    - [Web Configuration](/docs/plugins/social-Connexion/google/web/)
...
This section explains how the Google Sign-In integration with Supabase works under the hood. Understanding this flow will Aide you implement and troubleshoot the authentication process.
...
The implementation generates a secure nonce pair following the [Supabase nonce requirements](https://react-Natif-google-signin.github.io/docs/Sécurité#Utilisation-with-supabase):
...
1. **Prevent Invalid Requests**: If the token has an incorrect audience or nonce mismatch, Supabase will reject the token anyway. Validating first avoids unnecessary API calls and provides clearer Erreur messages.
...
3. **Sécurité** (iOS): On iOS, validation ensures the token was issued for your specific Google Client IDs, preventing potential Sécurité issues from using tokens intended for other applications.

4. **Better Erreur Handling**: Detecting issues before Supabase allows for automatic retry logic, which is essential for handling iOS caching issues transparently.

If validation fails, the function automatically:
1. Journaux out from Google (clears cached tokens - critical on iOS)
2. Retries authentication once (forces fresh token generation with correct nonce)
3. If retry also fails, Retourne an Erreur
...
- [SupabasePage.tsx](https://github.com/Cap-go/capacitor-social-Connexion/blob/main/Exemple-Application/src/SupabasePage.tsx) - Exemple component with redirect handling (Web)
...
Please proceed to the platform-specific Configuration Guide for your target platform:
