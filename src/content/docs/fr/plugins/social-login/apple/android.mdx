---
locale: fr
title: Apple Connexion on Android
description: This Guide provides a comprehensive walkthrough on setting up Apple Connexion using Capacitor for iOS Appareils, detailing each step to ensure a smooth integration process.
sidebar: 
    order: 3
---
...
<Steps>
1. Connexion into the [Apple Developer Portal](https://developer.apple.com). 
...
2. Click on `Identifiers`.
    ...
    1. Ensure that this field says <code>Application IDs</code>
...
<Steps>
   1. Go to [this website](https://developer.apple.com/Compte/) and scroll down
   
   2. Find the `Team ID`
      <img src="/social-login-assets/apple_dev_team_id.png" alt="Team ID location in developer account" />
</Steps>

## Configuring the Application redirect
...
A backend is required for Android, but configuring a backend will also impact iOS. An Exemple backend is provided [here](https://github.com/WcaleNieWolny/capgo-social-Connexion-backend-demo/blob/main/index.ts)
...
:::Remarque
If you already have an Application ID, you can skip this step.
Don't follow this step if you have configured Apple Connexion for iOS.
:::
