---
title: Premiers pas avec l'enregistreur audio
description: >-
  Découvrez comment intégrer l'enregistrement audio dans votre application
  Capacitor
locale: fr
---
...
Le plugin utilise le MediaRecorder API. Nécessite HTTPS en production.
...
Voici une implémentation complète de l'enregistreur vocal :
...
## Exemple d'intégration de l'interface utilisateur
...
1. **Demander les autorisations plus tôt** : vérifiez les autorisations avant d'afficher l'interface utilisateur d'enregistrement
2. **Gérer les interruptions** : les appels téléphoniques et les alarmes peuvent interrompre l'enregistrement
3. **Gérer le stockage** : nettoyez les anciens enregistrements pour économiser de l'espace
4. **Fournir des commentaires** : afficher l'état de l'enregistrement, la durée et la forme d'onde
5. **Test sur les appareils** : les simulateurs/émulateurs ont une prise en charge audio limitée
...
- Explorez la [API Reference](https://github.com/Cap-go/capacitor-audio-recorder#api) pour une documentation complète sur la méthode
- Consultez l'[exemple d'application](https://github.com/Cap-go/capacitor-audio-recorder/tree/main/example) pour une utilisation avancée.
- Voir le [tutoriel](/plugins/capacitor-audio-recorder) pour des exemples complets d'implémentation
