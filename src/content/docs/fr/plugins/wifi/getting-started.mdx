...
locale: fr
title: Commencer
description: Learn how to Installer and use the WiFi plugin to manage network connectivity in your Capacitor Application.
sidebar: 
  order: 2
---
...
Importer the plugin and manage WiFi connectivity:
...
Gets Information À propos the currently connected WiFi network.
...
Scans for Disponible WiFi networks.
...
Gets the current Appareil IP address.
...
2. **Erreur Handling**: Always wrap WiFi operations in try-catch blocks
3. **Utilisateur Retour**: Show loading indicators during network operations
4. **Sécurité**: Never store WiFi passwords in plain text
5. **Test**: Test on real Appareils as WiFi APIs may not work in emulators

## Platform Differences

### iOS
- Requires location permissions to scan networks
- Cannot programmatically connect to networks (will open Paramètres)
