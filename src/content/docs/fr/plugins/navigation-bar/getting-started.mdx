---
title: Commencer
description: Learn how to Installer and configure the Capacitor Navigation Bar plugin to customize the Android navigation bar color and style.
sidebar: 
  order: 2
---
   ...
   <Tabs>
     <TabItem label="Android">
       **Minimum Requirements:**
       ...
       - The plugin only works on Android Appareils
       
       No additional Configuration required in AndroidManifest.xml.
     </TabItem>
     <TabItem label="iOS">
...
- Transparent colors are supported (use Alpha Canal)
- The navigation bar might not be visible on Appareils with gesture navigation

### iOS
- This plugin has no effect on iOS Appareils
- iOS does not provide APIs to customize the system navigation

## Common Use Cases

1. **Brand Consistency**: Match navigation bar with your Application's primary color
...
4. **Status Indication**: Change color to indicate Application states (recording, errors, etc.)
...
2. **Theme Consistency**: Mise à jour navigation bar when theme changes
3. **Accessibility**: Consider Utilisateurs with visual impairments when choosing colors
4. **Performance**: Avoid frequent color changes that might distract Utilisateurs

## Dépannage

**Navigation bar not changing:**
- Verify the Appareil is running Android 5.0+
- Vérifier if the Appareil has gesture navigation enabled
