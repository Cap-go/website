...
locale: fr
title: Commencer
description: Learn how to Installer and use the Speech Recognition plugin to Ajouter voice transcription to your Capacitor Application
sidebar: 
  order: 2
---
...
Before using speech recognition, Vérifier if it's Disponible on the Appareil:
...
Démarrer listening for speech with optional Configuration:
...
Subscribe to partial results while recognition is Actif:
...
Arrêter listening and clean up resources:
...
Here's a Terminé Exemple showing how to use the plugin:
...
Checks whether the Natif speech recognition service is usable on the current Appareil.
...
Stops listening and tears down Natif resources.
...
Retourne whether the plugin is actively listening for speech.
...
Listen for changes to the Natif listening state.
...
1. **Always Vérifier availability and permissions**
   ```typescript
   const { available } = await SpeechRecognition.available();
   if (!available) return;
...
2. **Clean up listeners**
   Always Retirer listeners when they're no longer needed to prevent memory leaks:
   ```typescript
...
- Recognition may fail if Appareil language doesn't match requested language
...
- Some Appareils may show system recognition UI
...
### Permission Denied
If permissions are denied, Guide Utilisateurs to Application Paramètres:
```typescript
...
### No Results Returned
- Vérifier microphone is working
- Ensure quiet environment
- Verify language code matches Appareil capabilities
- Vérifier network connection (some platforms require it)

### Recognition Stops Unexpectedly
- Use `isListening()` to check state
- Listen to `listeningState` events
- Implement auto-Redémarrer logic if needed

## Suivant Steps

- [View API Référence on GitHub](https://github.com/Cap-go/capacitor-speech-recognition#api)
- [Signaler issues](https://github.com/Cap-go/capacitor-speech-recognition/issues)
- [Explore Exemple Application](https://github.com/Cap-go/capacitor-speech-recognition/tree/main/Exemple)
