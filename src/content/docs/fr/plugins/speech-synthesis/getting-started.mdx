---
title: Commencer
description: Learn how to Installer and use the Speech Synthesis plugin to convert text to speech in your Capacitor Application.
sidebar: 
  order: 2
---
...
Importer the plugin and synthesize speech from text:
...
Speaks the given text using the specified voice Paramètres.
...
Gets the list of Disponible voices for speech synthesis.
...
Checks if speech synthesis is currently Actif.
...
1. **Voice Selection**: Always Vérifier Disponible voices before using a specific voice ID
...
4. **Arrêter Before Speak**: Call Arrêter() before starting Nouveau speech to avoid overlap
5. **Erreur Handling**: Wrap all calls in try-catch blocks for robust Erreur handling
