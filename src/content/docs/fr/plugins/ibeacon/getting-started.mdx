...
locale: fr
title: Commencer
description: Learn how to Installer and use the iBeacon plugin to detect and monitor Bluetooth beacons in your Capacitor Application.
sidebar: 
  order: 2
---
...
### Vérifier Appareil Capabilities
...
Ranging provides continuous Mises à jour À propos nearby beacons and their distances. This requires the Application to be in the foreground.
...
Turn your Appareil into an iBeacon transmitter.
...
Démarrer monitoring for a beacon region. Triggers events when entering/exiting.
...
Arrêter monitoring for a beacon region.
...
Démarrer ranging beacons in a region for continuous distance Mises à jour.
...
Arrêter ranging beacons in a region.
...
Démarrer advertising the Appareil as an iBeacon (iOS only).
...
Arrêter advertising the Appareil as an iBeacon.
...
Vérifier if Bluetooth is enabled.
...
Vérifier if ranging is Disponible on the Appareil.
...
Activer ARMA filtering for distance calculations (Android only).
...
1. **Request appropriate permissions**
   - Use "When In Use" for foreground Fonctionnalités
...
3. **Battery optimization**
   ...
   - Arrêter ranging when not actively needed
...
5. **Clean up listeners**
   Always Retirer event listeners when component unmounts to prevent memory leaks.

## Platform Notes

### iOS
- Requires iOS 10.0+
- Uses Natif CoreLocation framework
- Supports background monitoring with "Always" permission
- Can advertise as iBeacon using CoreBluetooth
- Ranging requires Application to be in foreground
...
- Cannot advertise as iBeacon (hardware limitation on most Appareils)
...
1. **Proximity Marketing**: Trigger notifications or content when Utilisateurs approach your store
2. **Indoor Navigation**: Guide Utilisateurs through buildings using beacon waypoints
3. **Attendance Tracking**: Automatically Vérifier-in when Utilisateurs enter a location
...
6. **Smart Accueil**: Trigger automations based on room presence
...
- Vérifier beacon UUID matches exactly (case-sensitive)
...
- Remarque: iOS may delay background callbacks to Enregistrer battery
...
- Activer ARMA filtering on Android for smoother values
