...
Vous pouvez combiner ces options avec d'autres comme `--channel` selon vos besoins. Consultez la [documentation CLI Capgo](/docs/cli) pour tous les détails sur les options de téléchargement disponibles
...
Consultez la documentation [intégration CI/CD](/docs/cicd-integration) pour plus de conseils de dépannage. Vous pouvez également utiliser la commande `npx @capgo/cli@latest app debug` pour confirmer si vos mises à jour déclenchées par CI/CD sont bien reçues par l'application
