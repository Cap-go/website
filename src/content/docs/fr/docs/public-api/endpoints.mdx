...
Ceci est la documentation de l'API publique de Capgo cloud

Pour accéder, ajoutez dans les en-têtes votre clé API comme `authorization`
...
Ce point de terminaison vous permet de gérer les organisations et leurs membres

### GET

`https://apicapgoapp/organization`

Récupère les informations de l'organisation. Si `orgId` est fourni dans les paramètres, renvoie une seule organisation. Sinon, renvoie toutes les organisations accessibles
...
Ce point de terminaison vous permet de gérer les clés API pour accéder à l'API Capgo
...
Ce point de terminaison vous permet de récupérer diverses statistiques sur vos applications et organisations
...
Récupère les statistiques agrégées pour toutes les organisations auxquelles l'utilisateur a accès
...
Ce point de terminaison vous permet de vérifier et modifier tous les canaux disponibles de votre application
...
et optionnellement le nom du `channel` comme paramètre URL pour en recevoir un:
...
Ce point de terminaison vous permet de vérifier et modifier tous les appareils liés à votre application
...
Envoyez `app_id` comme paramètre URL et recevez un tableau des 50 premiers appareils\
Vous pouvez obtenir le suivant en envoyant `page=1`
...
Ce point de terminaison permet de délier le canal et la version forcée. Vous ne pouvez pas supprimer un appareil de Capgo
...
Supprimer un bundle dans le Cloud
