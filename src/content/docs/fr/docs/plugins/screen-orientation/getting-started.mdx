...
locale: fr
title: Commencer
description: Learn how to Installer and use the Screen Orientation plugin to control and detect Appareil orientation in your Capacitor Application.
sidebar: 
  order: 2
---
...
This plugin has a unique Fonctionnalité: it can detect the **true physical orientation** of the Appareil using motion sensors, even when the Utilisateur has enabled orientation lock on their Appareil.

<Aside type="note">
  The UI will still respect the Utilisateur's orientation lock setting. Motion tracking allows you to know how the Appareil is physically held, but the UI won't rotate if orientation lock is enabled.
</Aside>
...
Démarrer tracking physical Appareil orientation using motion sensors.
...
Arrêter motion-based orientation tracking.
...
Vérifier if Appareil orientation lock is enabled.
...
<Aside type="tip">
  This method requires motion tracking to be Actif to work properly.
</Aside>
