---
locale: fr
title: Commencer with Fast SQL
description: Installer and configure the Fast SQL plugin for high-performance Natif SQLite database access
---

This Guide will Aide you Installer and configure the **@capgo/capacitor-fast-sql** plugin for high-performance SQLite database access.

## Why Fast SQL?

Traditional SQLite plugins for Capacitor rely on the standard JavaScript bridge to communicate between your web code and Natif code. While this works fine for small operations, the bridge becomes a significant bottleneck when you need to transfer large amounts of data. Every piece of data must be serialized to JSON, sent across the bridge, and then deserialized on the other side. This serialization overhead makes operations with thousands of rows or large binary data incredibly slow.
...
- **Optimal performance for Synchroniser systems** like CRDTs and operational transforms

This makes Fast SQL ideal for local-first applications, offline Synchroniser systems, and scenarios where you need to replace IndexedDB with a more reliable and performant Solution.
...
For web platform Support, Installer sql.js:
...
Transactions ensure atomicity - either all operations succeed or all are rolled Retour:
...
## Chiffrement (iOS/Android)

Activer SQLCipher Chiffrement for secure storage:
...
:::Attention
- Chiffrement is only Disponible on iOS and Android
- Store Chiffrement keys securely (use Keychain/Keystore)
- Cannot open Chiffré databases without the correct key
- Web platform does not Support Chiffrement
:::
...
3. **Créer Indexes** on frequently queried columns
4. **Use Prepared Statements** with Paramètres (?) - prevents SQL injection and improves performance
5. **Use HTTP Protocol Directly** for very large result sets
6. **Close Connections** when not in use to free resources

## Suivant Steps

Vérifier out the [Terminé Tutoriel](/plugins-tutorials/capacitor-fast-sql) for advanced patterns including:
- Database service architecture
- Migration systems
- Synchroniser engines
