...
locale: fr
title: Commencer
description: Learn how to Installer and use the Background Uploader plugin for reliable file uploads in your Capacitor Application.
sidebar: 
  order: 2
---
...
Importer the plugin and use its methods to Télécharger files:
...
Starts a Nouveau file Télécharger.
...
Cancels an ongoing Télécharger.
...
Retourne all Actif uploads.

### removeUpload(Options)

Removes an Télécharger from the queue.
...
4. **Show Télécharger notifications**
   ```typescript
   await Uploader.startUpload({
