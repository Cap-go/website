...
<Aside type="caution" title="Les canaux n'apportent pas de confidentialité">
Les canaux contrôlent l'éligibilité à la mise à jour, pas le secret du bundle. Même si un canal est privé ou que l'auto-affectation est désactivée, tout bundle non chiffré envoyé à Capgo doit toujours être considéré comme un asset public livré aux clients. Le chiffrement protège la distribution et l'authenticité des mises à jour, mais les bundles distribués restent analysables par rétro-ingénierie depuis l'application avec suffisamment d'effort, car la clé publique fait partie du client. Voir [le chiffrement des Live Updates](/docs/live-updates/encryption/) pour le détail.
</Aside>
...
Versions minimales pour le changement de canal local uniquement : **5.34.0**, **6.34.0**, **7.34.0** ou **8.0.0** (selon votre version majeure). Chaque numéro de version mineure correspond au même ensemble de fonctionnalités sur toutes les versions majeures (par exemple, X.34.0 inclut les mêmes fonctionnalités que X soit 5, 6, 7 ou 8). Consultez [l'installation du plugin](/docs/getting-started/add-an-app/) pour les tags de version.
