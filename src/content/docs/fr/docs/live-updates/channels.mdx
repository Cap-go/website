...
Ouvrez votre fichier `capacitor.config.ts` (ou `capacitor.config.json`). Dans la section `plugins`, définissez la propriété `defaultChannel` du plugin `CapacitorUpdater` avec le nom de votre canal souhaité :
