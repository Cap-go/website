...
<Aside type="tip" title="Recommandé : Utiliser les Delta Updates avec Direct Update">
Lors de l'utilisation de `directUpdate`, nous **recommandons vivement** d'activer les [Delta Updates](/docs/live-updates/differentials/) pour minimiser les temps de téléchargement et améliorer l'expérience utilisateur. Les Delta Updates ne téléchargent que les fichiers modifiés au lieu du bundle entier, ce qui est particulièrement important lorsque les mises à jour sont appliquées immédiatement pendant que les utilisateurs utilisent activement votre application.

**Pourquoi c'est important pour les Direct Updates :**
- **Mises à jour plus rapides** : Des téléchargements plus petits signifient que les mises à jour se terminent rapidement, réduisant le temps où les utilisateurs voient des écrans de chargement
- **Meilleure expérience mobile** : Les utilisateurs sur réseaux cellulaires ou connexions plus lentes ne subiront pas de longs temps d'attente
- **Utilisation de bande passante réduite** : Seuls les fichiers modifiés sont téléchargés, économisant des données pour vous et vos utilisateurs

Pour activer les Delta Updates, utilisez simplement le flag `--partial` lors du téléchargement des bundles :
```shell
npx @capgo/cli@latest bundle upload --partial
```

En savoir plus dans la [documentation Delta Updates](/docs/live-updates/differentials/).
</Aside>
