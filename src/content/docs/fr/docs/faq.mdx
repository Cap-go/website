...
**Important** : Depuis la version du plugin **v6.25.0** et **v7.25.0**, l'ID de l'appareil persiste maintenant entre les réinstallations de l'application. Avant ces versions, chaque réinstallation de l'application générait un nouvel ID d'appareil et comptait comme un nouveau MAU.

Avec les versions actuelles :
- L'ID de l'appareil persiste entre les réinstallations de l'application (stocké de manière sécurisée dans le Keychain sur iOS et EncryptedSharedPreferences sur Android)
- La mise à jour de l'application ne crée pas un nouvel ID d'appareil
- Pendant le développement, si vous utilisez une version plus ancienne du plugin (< v6.25.0 / v7.25.0), chaque réinstallation crée toujours un nouveau MAU

Remarque : Les téléchargements TestFlight et les changements de canal sur Android peuvent toujours générer de nouvelles inscriptions d'appareils selon votre configuration.
...
Vous pouvez améliorer ou rétrograder votre forfait à tout moment dans votre tableau de bord : [https://console.capgo.app/settings/organization/plans](https://console.capgo.app/settings/organization/plans/)
...
**Persistance de l'ID de l'appareil (Mis à jour dans v6.25.0 et v7.25.0)** :
- **Comportement actuel** : L'ID de l'appareil persiste maintenant entre les réinstallations de l'application. Il est stocké de manière sécurisée dans le Keychain de l'appareil (iOS) ou EncryptedSharedPreferences (Android), ce qui nous permet de suivre le même appareil même après une désinstallation/réinstallation.
- **Comportement précédent** (avant v6.25.0/v7.25.0) : Pour des raisons de confidentialité liées aux politiques de l'App Store d'Apple et de Google, l'ID de l'appareil était réinitialisé à chaque réinstallation de l'application, rendant impossible le suivi du même appareil entre les réinstallations.

Les règles de confidentialité sont imposées par Apple et Google, et l'implémentation de Capgo est conforme à leurs meilleures pratiques pour l'identification des appareils.
