...
Si vous souhaitez gérer vous-même quand la mise à jour est appliquée, utilisez le mode manuel avec Capgo cloud

Voici ce que vous devez faire, configurez votre compte comme expliqué dans Démarrage
...
App.addListener('appStateChange', async (state: any) => {
  ...
  if (state.isActive) {
    ...
    // Effectuez le téléchargement pendant que l'utilisateur utilise l'application pour éviter les téléchargements échoués
