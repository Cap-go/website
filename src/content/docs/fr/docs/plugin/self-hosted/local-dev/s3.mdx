...
L'IP `hostdockerinternal` est une IP docker qui peut être atteinte uniquement en interne par docker, mais nous la remplaçons dans le code par `0000` pour que minio puisse être atteint depuis localhost
...
[^1]: Le fichier est situé dans votre répertoire d'application