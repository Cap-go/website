...
Anda dapat menggabungkan flag-flag ini dengan opsi lain seperti `--channel` sesuai kebutuhan. Lihat [dokumentasi CLI Capgo](/docs/cli) untuk detail lengkap tentang opsi upload yang tersedia

Jika Anda masih mengalami masalah dengan upload, hubungi [dukungan Capgo](mailto:support@capgo.app) untuk bantuan lebih lanjut
...
Lihat dokumentasi [integrasi CI/CD](/docs/cicd-integration) untuk tips troubleshooting lebih lanjut. Anda juga dapat menggunakan perintah `npx @capgo/cli@latest app debug` untuk mengkonfirmasi apakah pembaruan yang dipicu CI/CD diterima oleh aplikasi
