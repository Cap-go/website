---
title: Perintah
description: Dokumentasi CLI Capgo
sidebar:
  order: 2
locale: id
---
...
Semua perintah harus dijalankan di folder aplikasi dengan proyek Capacitor yang telah diinisialisasi dengan benar
...
Metode ini ada untuk membantu Anda langkah demi langkah

Ini akan menambahkan aplikasi Anda ke Capgo, menambahkan kode ke aplikasi Anda untuk memvalidasi pembaruan, membangun aplikasi Anda, mengunggah aplikasi Anda ke Capgo dan membantu Anda memeriksa apakah pembaruan berfungsi
...
:::note
gunakan `--apikey=********` di perintah manapun untuk mengganti
:::

**Secara opsional Anda dapat memberikan:**

`--local` Ini akan menyimpan **apikey** Anda di repo lokal dan mengabaikannya di git
...
`[path]` - path pengaturan yang ingin Anda ubah. Misalnya, untuk mengubah `appId`, berikan `appId`
...
* `--channel <channel>` Channel untuk dihubungkan
...
* `--s3-endpoint <s3Endpoint>` URL endpoint S3. Tidak berfungsi dengan unggahan Parsial atau opsi eksternal
...
* `--key-v2 <key>` Path kustom untuk kunci penandatanganan pribadi (sistem v2)
* `--key-data-v2 <keyData>` Kunci penandatanganan pribadi (sistem v2)
* `--bundle-url` Mencetak URL bundle ke stdout
* `--no-key` Abaikan kunci penandatanganan dan kirim pembaruan yang jelas
* `--no-code-check` Abaikan pemeriksaan apakah notifyAppReady() dipanggil dalam kode sumber dan index ada di folder root
...
* `--min-update-version <minUpdateVersion>` Versi minimal yang diperlukan untuk memperbarui ke versi ini. Hanya digunakan jika pembaruan otomatis dinonaktifkan ke metadata di channel
...
* `--multipart` Menggunakan protokol multipart untuk mengunggah data ke S3, Sudah usang, gunakan TUS sebagai gantinya
...
> ⭐️ Opsi eksternal membantu membuka 2 kasus: perusahaan dengan masalah privasi, jangan kirim kode ke pihak ketiga dan aplikasi lebih besar dari 200 MB. Dengan pengaturan ini, Capgo hanya menyimpan tautan ke zip dan mengirim tautan ke semua aplikasi

> 👀 Cloud Capgo tidak pernah melihat apa yang ada di tautan (untuk opsi eksternal), atau di kode saat disimpan
...
> 💡 Jangan lupa untuk memperbarui nomor versi setiap kali Anda mengirim satu, nomor versi tidak dapat ditimpa, atau digunakan kembali setelah penghapusan karena alasan keamanan
...
dalam rentang SemVer untuk versi utama ke Cloud
...
Contoh: Jika Anda memiliki 10 versi dari 1001 hingga 10011, dan Anda menggunakan `npx @capgo/cli cleanup [appId] --bundle=1000` ini akan menghapus 1001 hingga 1006, 1007 sampai 10011 akan disimpan

Jika Anda memiliki total 20 versi, dan Anda tidak memberikan nomor bundle seperti ini: `npx @capgo/cli cleanup [appId] --keep=2` Ini akan menghapus 18 versi, dan menyimpan 2 yang terakhir
...
> **Peringatan**: Perintah ini sudah usang dan akan dihapus di rilis mayor berikutnya. Silakan gunakan sistem enkripsi baru
...
`--key [/path/to/my/private_key]` path ke private key Anda
`--key-data [privateKey]` data private key, jika Anda ingin menggunakan secara inline
Perintah ini akan mencetak `ivSessionKey` Anda dan menghasilkan zip terenkripsi, untuk digunakan dengan perintah upload atau decrypt
...
Dalam enkripsi v2 checksum ditingkatkan menjadi "tanda tangan" dari bundle
...
`--key-data [privateKey]` data private key, jika Anda ingin menggunakan secara inline Perintah ini terutama digunakan untuk tujuan pengujian, akan mendekripsi zip dan mencetak session key base64 yang telah didekripsi di konsol
...
`[channelId]` nama channel baru Anda `[appId]` ID aplikasi Anda format `comtestapp` dijelaskan [di sini](https://capacitorjscom/docs/cli/commands/init/)

### **Delete**

`npx @capgo/cli channel delete [channelId] [appId]`

`[channelId]` nama channel yang ingin Anda hapus `[appId]` ID aplikasi Anda format `comtestapp` dijelaskan [di sini](https://capacitorjscom/docs/cli/commands/init/)

### **List**

`npx @capgo/cli channel list [appId]`

`[appId]` ID aplikasi Anda format `comtestapp` dijelaskan [di sini](https://capacitorjscom/docs/cli/commands/init/)
...
* `--latest` dapatkan versi bundle dari `packagejson:version`, tidak dapat digunakan dengan `--bundle`
* `--state [ normal | default ]` atur status channel, bisa `normal` atau `default` Satu channel harus `default`
...
* `--self-assign` mengizinkan perangkat untuk mengatur diri sendiri ke channel ini
* `--no-self-assign` melarang perangkat untuk mengatur diri sendiri ke channel ini
* `--disable-auto-update STRATEGY` Nonaktifkan strategi pembaruan otomatis untuk channel ini Opsi yang mungkin adalah: major, minor, metadata, none
* `--apikey [key]` API key untuk menghubungkan ke akun Anda
...
**PERHATIKAN** strategi ini tidak mencegah pembaruan dari `010` -> `110`
...
Berikut contoh skenario dimana pembaruan diizinkan atau ditolak
...
Terakhir strategi paling rumit Strategi `metadata`\
Pertama Anda perlu tahu bahwa awalnya setelah Anda mengaktifkannya pembaruan **AKAN** gagal karena channel kekurangan metadata yang diperlukan\
Jika channel kekurangan metadata Anda akan melihat pesan seperti ini:
...
Kemudian pergi ke channel yang gagal dan klik `Bundle number`. Ini akan membawa Anda ke halaman bundle
<img src="/fail-channel-showwebp" alt="Locate failing channel"/>

Setelah di sana, isi kolom `Minimal update version`. Ini harus berupa [semver](https://devhints.io/semver/)\
Jika nilai yang Anda masukkan bukan semver, Anda akan mendapat error, tetapi jika semuanya berjalan dengan benar, Anda akan melihat seperti ini:
...
Untuk mengunggah bundle dengan benar saat menggunakan opsi `metadata`, Anda perlu menyertakan `--min-update-version` dengan semver yang valid. Seperti ini:
<img src="/cli-upload-with-metadatawebp" alt="CLI upload with metadata"/>

`--min-update-version` bukanlah satu-satunya cara untuk melakukan kompatibilitas
...
Kedua, jika versi baru 100% kompatibel, menggunakan kembali `min_update_version` dari versi terbaru di channel
Jika tidak, maka akan mengatur `min_update_version` ke nomor bundle versi yang baru diunggah

Anda akan selalu mendapat informasi apa `min_update_version` saat menggunakan opsi ini. Tampilan akan seperti ini:
<img src="/min_update_version_infowebp" alt="Min update version"/>

Jika versi baru tidak kompatibel, tampilan akan seperti ini
...
Jangan lupa mengkonfigurasi variabel lingkungan CI dengan API key Anda