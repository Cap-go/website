...
description: Cara mengkonfigurasi minio untuk digunakan dengan capgo
sidebar:
  order: 2
locale: id
---
...
Sekarang setelah kita memiliki server S3 yang berjalan, kita perlu mengatur fungsi edge capgo untuk menggunakan server S3 kita

Untuk melakukan itu kita perlu membuat file ENV di `capgo/supabase` bernama `envlocal`

File ini harus terlihat seperti ini:
```bash
