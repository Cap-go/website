...
description: Cara menggunakan capacitor updater dengan capgo yang di-host sendiri
sidebar:
  order: 4
locale: id
---
...
1 [Capgo yang sudah diclone](https://githubcom/Cap-go/capgo)
...
Ini dapat dicapai dengan memodifikasi [AndroidManifestxml](https://githubcom/Cap-go/capacitor-updater/blob/main/android/src/main/AndroidManifestxml) dan menambahkan `android:usesCleartextTraffic="true"` di tag `application`
Contoh lengkap perubahan ini dapat dilihat [di sini](https://gistgithubcom/WcaleNieWolny/061a015acdebe35eaf3afd7030797701)

Karena secara default, Android tidak mengizinkan `http` capgo telah menerapkan pemeriksaan untuk http [di sini](https://githubcom/Cap-go/capgo/blob/4cce6c0707d745faa2afbab643e636bad927c602/supabase/functions/updates/indexts#L396)
Kode ini mungkin tidak selalu ada di sini, tapi untuk sekarang perubahan pada baris ini diperlukan agar pembaruan berfungsi  
...
Kode ini ada di sini [dengan sengaja](https://githubcom/Cap-go/capacitor-updater/issues/37#issuecomment-1684916354)
:::

Mungkin juga ada masalah yang mencegah aplikasi android untuk terhubung. Jika Anda tidak melihat permintaan yang dikirim ke edge functions jalankan
```bash
