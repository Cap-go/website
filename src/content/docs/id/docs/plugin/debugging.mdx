---
title: デバッグ
description: Cara melakukan debug aplikasi Anda
sidebar:
  order: 6
locale: id
---
...
| **delete**            | Satu bundle telah dihapus di perangkat                                          |
| **set**               | Sebuah bundle telah diatur di perangkat                                         |
| **set_fail**          | Bundle gagal diatur                                                             |
| **reset**             | Perangkat direset ke bundle `builtin`                                           |
| **download_XX**       | Bundle baru telah diunduh - progres ditunjukkan oleh XX% (kelipatan 10%)       |
| **download_complete** | Bundle baru selesai diunduh                                                     |
| **download_fail**     | Bundle baru gagal diunduh                                                       |
| **update_fail**       | Bundle baru telah diinstal tetapi gagal memanggil `notifyAppReady`             |
| **checksum_fail**     | Bundle baru gagal memvalidasi checksum                                          |
| **windows_path_fail** | Zip memiliki file yang berisi path windows yang ilegal                         |
...
* `Failed to download from` **=>** sama dengan **download\_fail**
* `notifyAppReady was not called, roll back current bundle` => sama dengan **update\_fail**

## Menemukan bundle yang diunduh di perangkat Anda

### iOS

Untuk debug di iOS, Anda perlu mengambil aplikasi di komputer Anda, Anda dapat melakukannya seperti ini:

Xcode memiliki fitur bawaan untuk memeriksa sistem file aplikasi yang diinstal pengembang di perangkat iOS

Untuk mencapainya:

* Hubungkan perangkat Anda ke Mac dan pilih Window > Devices di menu Xcode
* Pilih perangkat Anda di panel kiri di bawah bagian Devices
* Ini akan menampilkan daftar aplikasi yang diinstal pengembang untuk perangkat tersebut
* Pilih aplikasi yang ingin Anda periksa dan pilih ikon roda gigi di dekat bagian bawah layar
* Di sini Anda dapat melihat sistem file saat ini dengan memilih Show Container atau mengunduh snapshot
...
Buka folder App Data, dan Anda seharusnya sekarang melihat beberapa folder seperti Documents, Library, tmp, dll
...
* Klik View > Tool Windows > Device File Explorer atau klik tombol Device File Explorer di bar jendela alat untuk membuka Device File Explorer
* Pilih perangkat dari daftar dropdown
* Buka path **data/data/APP\_NAME/** dimana **APP\_NAME adalah ID aplikasi Anda**
