...
Sie fügt Ihre App zu Capgo hinzu und implementiert den Code zur Validierung der Aktualisierung. Außerdem wird Ihre App erstellt, zu Capgo hochgeladen und hilft Ihnen zu überprüfen, ob die Aktualisierung funktioniert
...
Befehl zur Überprüfung, ob Ihre Capgo-Pakete aktuell sind

Dieser Befehl ist auch nützlich für Fehlerberichte
...
> 💡 Alle Optionen werden aus Ihrer Konfiguration ermittelt, wenn nicht angegeben
...
Bearbeitet die Capacitor-Konfiguration
...
* `--external <url>` Link zu externer URL statt Upload zur Capgo Cloud
* `--iv-session-key <key>` IV und Session-Key für Bundle-URL extern setzen
* `--s3-endpoint <s3Endpoint>` URL des S3-Endpunkts Funktioniert nicht mit Partial Upload oder externer Option
...
* `--no-key` Ignoriert Signierungsschlüssel und sendet klares Update
* `--no-code-check` Ignoriert die Überprüfung, ob notifyAppReady() im Quellcode aufgerufen wird und index im Root-Ordner vorhanden ist
* `--display-iv-session` Zeigt in der Konsole den verwendeten IV und Session-Key für die Verschlüsselung
* `--bundle <bundle>` Bundle-Versionsnummer des hochzuladenden Bundles
* `--min-update-version <minUpdateVersion>` Minimal erforderliche Version für das Update auf diese Version Wird nur verwendet, wenn auto update auf metadata im Kanal gesetzt ist
* `--auto-min-update-version` Setzt die minimale Update-Version basierend auf nativen Paketen
* `--ignore-metadata-check` Ignoriert die Metadata-Prüfung (node_modules) beim Upload
* `--ignore-checksum-check` Ignoriert die Checksum-Prüfung beim Upload
* `--timeout <timeout>` Timeout für den Upload-Prozess in Sekunden
* `--partial` Lädt keine Teiledateien zur Capgo Cloud hoch
* `--tus` Lädt das Bundle über das tus-Protokoll hoch
* `--multipart` Verwendet Multipart-Protokoll für den Upload zu S3, Veraltet, verwenden Sie stattdessen TUS
* `--encrypted-checksum <encryptedChecksum>` Eine verschlüsselte Prüfsumme (Signatur) Nur verwendet beim Upload eines externen Bundles
* `--package-json <packageJson>` Ein Pfad zur packagejson Nützlich für Monorepos
...
> ⭐️ Die externe Option hilft bei 2 Fällen: Unternehmen mit Datenschutzbedenken, die keinen Code an Dritte senden möchten, und Apps größer als 200 MB. Mit dieser Einstellung speichert Capgo nur den Link zur ZIP-Datei und sendet den Link an alle Apps
...
in einem SemVer-Bereich für eine Major-Version zur Cloud
...
* `--apikey [key]` API-Schlüssel zur Verknüpfung mit Ihrem Konto
* `--bundle [majorVersion]` eine Version, für die Sie vorherige Pakete entfernen möchten, es wird die letzte + `numberToKeep` behalten* `--keep [numberToKeep]` die Anzahl der Pakete, die Sie behalten möchten (Standard 4)

Zum Beispiel: Wenn Sie 10 Versionen von 1001 bis 10011 haben und Sie `npx @capgo/cli cleanup [appId] --bundle=1000` verwenden, werden 1001 bis 1006 entfernt und 1007 bis 10011 werden behalten

Wenn Sie insgesamt 20 Versionen haben und keine Bundle-Nummer angeben, wie hier: `npx @capgo/cli cleanup [appId] --keep=2` werden 18 Versionen entfernt und die letzten 2 behalten
...
Dieser Befehl wird verwendet, wenn Sie externe Quellen zum Speichern Ihres Codes verwenden oder zu Testzwecken
Die Prüfsumme ist der SHA256 des Bundles (generiert durch --key-v2) und wird verwendet, um die Integrität der Datei nach der Entschlüsselung zu überprüfen
...
`[channelId]` der Name des Kanals, den Sie löschen möchten `[appId]` Ihre App-ID, das Format `comtestapp` wird [hier](https://capacitorjscom/docs/cli/commands/init/) erklärt
...
* `--bundle [123]` Ihr bereits in die Cloud hochgeladenes App-Bundle, um es mit einem Kanal zu verknüpfen
...
## Deaktivieren der Update-Strategie

Es gibt mehrere Möglichkeiten, Updates für zu alte Versionen zu deaktivieren\
Capgo kann nativen Code nicht aktualisieren, daher sollte ein Update von einer Version mit altem nativen Code zu einer Version mit aktualisiertem nativen Code nicht möglich sein
Es gibt mehrere Möglichkeiten, dies zu erreichen

Erstens die `major` Strategie. Sie verhindert ein Update von `000` -> `100`. Die Major-Version ist die hervorgehobene Zahl (**1**00 und **0**00)\
Zweitens die `minor` Strategie. Sie verhindert ein Update von `000` -> `110` oder ein Update von `110` auf `120`
**ACHTUNG** diese Strategie verhindert kein Update von `010` -> `110`
...
Damit ein Update akzeptiert wird, müssen folgende Bedingungen erfüllt sein:
 - Die Major-Version ist zwischen der neuen und alten Version gleich
 - Die Minor-Version ist zwischen der neuen und alten Version gleich
 - Die Patch-Version der neuen Version ist größer als die Patch-Version der alten Version

Hier ein Beispiel, in welchen Szenarien das Update erlaubt oder verweigert wird:
...
Zunächst müssen Sie wissen, dass die Updates nach der Aktivierung zunächst **FEHLSCHLAGEN** werden, da dem Kanal die erforderlichen Metadaten fehlen\
...
Finden Sie zunächst heraus, welcher Kanal fehlschlägt. Sie können das in der Spalte `misconfigured` sehen
...
Wenn der eingegebene Wert kein semver ist, erhalten Sie einen Fehler. Bei korrekter Eingabe sollten Sie etwa Folgendes sehen:
<img src="/set-min-update-versionwebp" alt="Set min version"/>

Sie möchten diese Daten wahrscheinlich nicht bei jedem Update manuell eingeben. Glücklicherweise verhindert die CLI das Senden eines Updates ohne diese Metadaten.
...
Zunächst wird die aktuell im Kanal hochgeladene Version überprüft. Die Kompatibilität wird wie beim Befehl `bundle compatibility` geprüft.
Zweitens, wenn die neue Version 100% kompatibel ist, wird die `min_update_version` der neuesten Version im Kanal wiederverwendet.
...
## Ende-zu-Ende-Verschlüsselung (Vertrauenslos)
...
Um Ihre Arbeit zu automatisieren, empfehle ich, GitHub Actions die Aufgabe des Pushens zu unserem Server zu überlassen.
