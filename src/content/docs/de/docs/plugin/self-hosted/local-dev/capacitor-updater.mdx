...
description: So verwendet man den Capacitor Updater mit selbst gehostetem Capgo
sidebar:
  order: 4
locale: de
---
...
Dies ermöglicht die Nutzung von lokalem Capgo in der Entwicklung. Allerdings reicht das standardmäßig nicht aus.

Für Android ist die Aktivierung von [Klartext-Kommunikation](https://developerandroidcom/topic/security/risks/cleartext) erforderlich, da SSL standardmäßig lokal deaktiviert ist.
...
Da Android standardmäßig `http` nicht erlaubt, hat Capgo eine Überprüfung für http [hier](https://githubcom/Cap-go/capgo/blob/4cce6c0707d745faa2afbab643e636bad927c602/supabase/functions/updates/indexts#L396) implementiert.
Dieser Code ist möglicherweise nicht immer hier, aber im Moment ist eine Änderung dieser Zeile erforderlich, damit Updates funktionieren.
