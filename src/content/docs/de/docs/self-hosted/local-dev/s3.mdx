...
:::danger
⚠️ Dieses Tutorial sollte nicht in der Produktion verwendet werden\
...
Dann erstellen Sie einen Ordner namens `data` darin.
...
Wenn Sie das Konsolenfenster mit diesem Container jemals schließen, können Sie es mit folgendem Befehl starten:
```bash
docker start minio1
```
...
Jetzt, da wir einen S3-Server ausführen, müssen wir die capgo Edge-Funktionen einrichten, um unseren S3-Server zu verwenden.

Um das zu tun, müssen wir eine ENV-Datei in `capgo/supabase` mit dem Namen `envlocal` erstellen.
...
Die IP `hostdockerinternal` ist eine Docker-IP, die nur von Docker intern erreicht werden kann, aber wir ersetzen sie im Code durch `0000`, damit minio von localhost erreicht werden kann.
...
const config: CapacitorConfig = {
  ...
  plugins: {
    CapacitorUpdater : {
      // Ohne dieses localS3 schlägt der Upload-Befehl fehl
      localS3: true
    },
  },
};
