...
업데이트 엔드포인트에 새 버전이 없을 때는 다음과 같은 에러 페이로드로 응답해야 합니다:

```json
{ "error": "no_new_version_available", "message": "No new version available" }
```

`error` 코드는 반드시 `no_new_version_available` 여야 합니다. `message`는 어떤 문자열이든 가능하며(로그/디버그용) 기대되는 동작입니다. 이 응답은 HTTP `200`으로 반환됩니다. `url` 없이 `200`을 반환하면 플러그인이 다운로드 실패로 처리하고 `download_fail`을 전송합니다.
