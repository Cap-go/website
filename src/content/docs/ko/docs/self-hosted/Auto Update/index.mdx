...
description: 자체 호스팅 모드에서 자동 업데이트 플러그인 사용 방법
sidebar:
  order: 1
locale: ko
---
...
이 작업을 직접 사용하시는 경우, [여기](https://githubcom/sponsors/riderx/)를 통해 제 작업을 지원해주시길 강력히 추천드립니다.

여기에 구축한 소중한 코드를 모두 오픈소스로 공개하는 큰 결정을 내렸습니다.

이를 저만 보유하고 높은 가격을 책정할 수도 있었습니다.
...
capgo의 제안이 적합하지 않다면, [여기](https://githubcom/sponsors/riderx/)에서 원하시는 가격으로 부트스트랩 제작자를 지원해주세요.
...
> 🚧 안드로이드에서는 HTTP 요청이 허용되지 않습니다. 테스트 목적으로는 [이를 허용](https://stackoverflowcom/questions/45940861/android-8-cleartext-http-traffic-not-permitted/)하세요.

### 업데이트 API

앱이 열릴 때마다 플러그인이 다음 본문으로 API에 POST 요청을 보냅니다:
...
업데이트가 필요한 경우 서버 API는 capacitor-updater 플러그인에 다음 JSON 데이터로 응답해야 합니다:
...
자동 업데이트에서는 서버가 버전을 비교하고 올바른 버전을 반환하는 작업을 수행해야 합니다. URL 키가 있으면 플러그인이 다운로드 프로세스를 시작합니다.
...
버전 4150부터 플러그인은 암호화된 업데이트를 보낼 수 있습니다.

개인 키를 생성하여 이 기능을 사용할 수 있습니다 ``npx @capgo/cli key create``
...
그러면 앱이 개인 키를 사용하여 `session_key`를 복호화하고 복호화된 `session_key`를 사용하여 업데이트를 복호화할 수 있습니다.
...
interface AppInfosStats {
	"action": "set", // set, delete, set_fail, reset, revert가 될 수 있음
	// 그 다음은 업데이트와 동일한 정보
	"app_id": "*******", // 스토어의 앱 식별자
	...
	"version_os": "16", // 기기의 OS 버전
        "plugin_version": "400"// 다른 플러그인에 따라 API가 다르게 동작하도록 함
        "is_emulator": false,
    	"is_prod": false,
}
```

빈 문자열로 완전히 비활성화할 수도 있습니다. 통계는 개인정보 보호에 중점을 두고 있으며 사용자들이 플러그인을 어떻게 사용하는지 이해하고 문제를 해결하고 개선하는 데 도움이 된다는 점을 기억하세요.
