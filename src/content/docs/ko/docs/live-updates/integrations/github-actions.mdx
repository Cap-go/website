...
description: "앱 업데이트의 자동 배포를 위해 Capgo Live Updates를 GitHub Actions와 통합하는 방법을 알아보세요."
sidebar:
  order: 3
locale: ko
---
...
GitHub Actions 통합을 설정하기 전에 다음을 준비해야 합니다:
...
GitHub 저장소에 필요한 시크릿을 설정합니다:

<Steps>

1. GitHub 저장소로 이동
2. **Settings** → **Secrets and variables** → **Actions**로 이동
3. **New repository secret**을 클릭하고 다음 추가:
...
main 브랜치에 푸시할 때마다 프로덕션에 배포하는 기본 구성으로 시작:

```yaml
# Capgo Live Updates를 위한 간단한 GitHub Actions 워크플로우
      ...
      - name: Install, test and build
        run: |
         ...
         run: |
           ...
           # 암호화된 업로드의 경우 추가: --key-data-v2 "${{ secrets.CAPGO_PRIVATE_KEY }}"
```
...
로컬에서 [암호화 키 설정](/docs/live-updates/encryption/#setting-up-encryption) 후 GitHub 시크릿에 개인 키를 추가하세요:
...
이 콘텐츠를 GitHub 저장소 시크릿에 `CAPGO_PRIVATE_KEY`로 추가한 다음 워크플로우에서 사용:
...
**채널로 테스트**: PR 또는 개발 채널에 배포한 후 해당 특정 채널을 사용하도록 구성하여 앱에서 업데이트를 테스트할 수 있습니다. [앱에서 채널 구성](/docs/live-updates/channels/#configuring-the-channel-in-your-app)에 대해 자세히 알아보세요.

</Aside>

### 기능 채널 정리

브랜치가 삭제될 때 기능 채널을 자동으로 정리:
...
**"CAPGO_TOKEN not found"로 워크플로우 실패:**
...
프로젝트 간 일관성을 위한 재사용 가능한 워크플로우 생성:
