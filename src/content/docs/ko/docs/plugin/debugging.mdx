...
| 코드                          | 설명                                                                                                                                                                 |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **InvalidIp**                                    | 사용자가 Google 데이터 센터에 위치해 있고 업데이트가 4시간 미만인 경우입니다. 이는 Google 봇의 기기가 계정의 기기로 집계되는 것을 방지하기 위함입니다 |
...
| **disableAutoUpdateUnderNative**                 | 기기가 버전(`123`)을 사용 중이고, 채널에 기기 버전 아래로 보낼 업데이트(`122`)가 있지만 채널 설정에서 비활성화되어 있습니다                    |
...
| **delete**            | 기기에서 번들이 하나 삭제되었습니다                                               |
| **set**               | 기기에 번들이 설정되었습니다                                                       |
| **set_fail**          | 번들 설정에 실패했습니다                                                          |
| **reset**             | 기기가 `builtin` 번들로 초기화되었습니다                                          |
| **download_XX**       | 새 번들이 다운로드됨 - XX%로 진행률 표시 (10% 단위로 증가)                        |
...
Xcode에는 iOS 기기에 설치된 개발자 앱의 파일 시스템을 검사하는 기능이 내장되어 있습니다

방법:

* Mac에 기기를 연결하고 Xcode 메뉴바에서 Window > Devices 선택
* Devices 섹션의 왼쪽 창에서 기기 선택
* 해당 기기의 개발자 설치 앱 목록이 표시됩니다
* 검사하려는 앱을 선택하고 화면 하단의 기어 아이콘 선택
* Show Container를 선택하여 현재 파일 시스템을 보거나 스냅샷을 다운로드할 수 있습니다

Download Container를 선택하면 파일 시스템의 스냅샷을 xcappdata 파일로 다운로드하고 내보냅니다
...
* 드롭다운 목록에서 기기를 선택합니다
* **data/data/APP\_NAME/** 경로를 엽니다. 여기서 **APP\_NAME은 앱 ID**입니다

![image](https://user-imagesgithubusercontentcom/4084527/166708728-8f96fc73-5d90-426f-8d27-301697347a5fwebp/)

그런 다음 모든 버전을 보려면 `versions` 폴더를 찾으세요

:::tip[알고 계셨나요?]
Android에서는 모든 버전이 하나의 폴더에 저장되지만, iOS에서는 두 위치에 복제해야 합니다
:::
