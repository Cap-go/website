...
description: 업데이트 자체적으로 관리하는 방법
sidebar:
  order: 5
locale: ko
---
...
Capgo 클라우드에서 수동 모드를 사용하여 업데이트가 적용되는 시기를 직접 관리하고 싶다면
...
App.addListener('appStateChange', async (state: any) => {
  ...
  if (!state.isActive && data) {
    console.log('set')
    // 사용자가 앱을 종료할 때나 원하는 시점에 전환을 수행합니다
    SplashScreen.show()
    try {
      ...
      console.log(err)
      SplashScreen.hide() // set이 실패한 경우, 그렇지 않으면 새 앱에서 숨겨야 합니다
    }
  }
})
```

플러그인에서 사용 가능한 모든 API 문서:
...
사용자가 채널을 구독하고 다른 버전을 시도할 수 있도록 하는 몇 가지 사용 사례가 있습니다:\
