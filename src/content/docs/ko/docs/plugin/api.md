...
| **`localApiFiles`**          | <code>string</code>  | 테스트를 위한 로컬 파일 API 사용 설정                                                                                                                                          | <code>undefined</code>                             | 633   |
...
<docgen-api>
<!--소스 파일 JSDoc 주석을 업데이트하고 docgen을 다시 실행하여 아래 문서를 업데이트하세요-->
...
업데이트가 필요없는 이벤트 수신, 앱이 실행될 때마다 강제로 체크하려는 경우 유용함
