---
title: "Bundles"
description: "앱 업데이트 패키지의 목록 표시, 삭제, 버전 관리 및 스토리지 최적화를 다루는 Capgo Bundles 관리를 위한 상세 가이드."
sidebar:
  order: 8
---
...
Capgo CLI를 사용하지 않고 수동으로 번들을 생성하고 업로드하는 방법:
...
### 단계 2: Capgo CLI와 동일한 패키지를 사용하여 Bundle Zip 생성

**중요**: 호환성을 보장하기 위해 Capgo CLI가 내부적으로 사용하는 것과 정확히 동일한 JavaScript 패키지를 사용하세요.
...
#### JavaScript로 Zip Bundle 생성 (Capgo CLI와 동일)
...
### 단계 3: CLI와 동일한 패키지를 사용하여 SHA256 체크섬 계산
...
### JavaScript 체크섬 계산 (Capgo CLI와 동일)
