...
const pluginDocsIndexPromise: Promise<Map<Locales, Set<string>>> = getCollection('docs').then((entries) => {
  const docsByLocale = new Map<Locales, Set<string>>()

  for (const entry of entries) {
    const docEntry = getPluginDocEntry(entry.filePath)
...
const normalizeFilePath = (filePath: string): string => {
  ...
  return docsRootIndex >= 0 ? normalizedPath.slice(docsRootIndex + docsRoot.length) : normalizedPath
}
...
export const getPluginDocsSlugs = async (locale?: string): Promise<PluginDocsSlugs> => {
  ...
  const normalizedLocale = locales.includes(locale as Locales) ? (locale as Locales) : defaultLocale
  const defaultDocsSlugs = docsByLocale.get(defaultLocale) ?? new Set<string>()
