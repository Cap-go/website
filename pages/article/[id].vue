...
const { data: randomData } = await useAsyncData('randomData', async () => {
  return data.value?.next_blog && data.value?.next_blog !== ''
    ? await queryContent<MyCustomParsedContent>('article', data.value.next_blog)
      .findOne()
...
const structuredData: WithContext<NewsArticle> = {
  ...
  'mainEntityOfPage': {
    '@type': 'WebPage',
    '@id': `${config.public.domain}/${data.value?.slug}`,
  },
  'headline': data.value?.description,
  'image': [
    `${config.public.domain}${data.value?.head_image || '/capgo_banner.webp'}`,
  ],
  ...
  'publisher': {
    ...
    'logo': {
      '@type': 'ImageObject',
      'url': `${config.public.domain}/icon.webp`,
    },
  },
}
      ...
      <div class="relative mx-4 flex">
        <div :title="randomData.title" class="block w-full">
          <img
            ...
            :alt="`article illustration ${randomData.title}`"
          >
        </div>
