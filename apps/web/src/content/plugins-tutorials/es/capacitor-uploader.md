...
Este tutorial te guiará a través del proceso de uso del paquete `@capgo/capacitor-uploader` para subir archivos de forma nativa en tu aplicación Ionic Capacitor.
...
3 Después de la instalación, sincroniza tu proyecto Capacitor:
...
Para Android, necesitas agregar algunos permisos a tu archivo `AndroidManifestxml`. Abre el archivo ubicado en `android/app/src/main/AndroidManifestxml` y añade los siguientes permisos dentro de la etiqueta `<manifest>`:
...
### Usando con Capacitor Camera Preview

Si estás usando el plugin Capacitor Camera Preview, puedes combinarlo con el Uploader para capturar y subir videos. Aquí hay un ejemplo:
...
Ahora has aprendido cómo usar el paquete `@capgo/capacitor-uploader` para subir archivos de forma nativa en tu aplicación Ionic Capacitor. Este plugin proporciona una forma flexible de subir archivos a varios servidores, incluido S3 con URLs prefirmadas, y se puede utilizar en combinación con otros plugins como el Capacitor Camera Preview.

Recuerda manejar los errores apropiadamente y gestionar los eventos de subida para proporcionar retroalimentación a tus usuarios sobre el progreso y estado de la subida.
