  ...
  const metrics = await getPublicLiveUpdateMetrics({
    ...
    fetch: async (_url, init) => {
      ...
      if (query.includes('has_reset'))
        return analyticsResponse([{ rollbacks: '3', outcomes: '100' }])
      if (query.includes('zip_successes'))
        return analyticsResponse([{ zip_successes: '40', zip_failures: '10', delta_successes: '80', delta_failures: '5' }])
      ...
      if (query.includes('platform AS key') && query.includes('action, count()'))
        return analyticsResponse([{ key: 'android', action: 'download_fail', devices: '12' }])
      if (query.includes('FROM device_info') && query.includes('blob10'))
        return analyticsResponse([{ key: 'US', devices: '70' }])
      if (query.includes('country AS key') && query.includes('sum(succeeded)'))
        return analyticsResponse([{ key: 'US', successes: '90', failures: '10' }])
      if (query.includes('country AS key') && query.includes('action, count()'))
        return analyticsResponse([{ key: 'US', action: 'unzip_fail', devices: '2' }])
      if (query.includes('FROM device_info') && query.includes('blob3'))
        return analyticsResponse([{ key: '8.1.0', devices: '60' }])
      if (query.includes('plugin_version AS key') && query.includes('sum(succeeded)'))
        return analyticsResponse([{ key: '8.1.0', successes: '70', failures: '5' }])
      if (query.includes('plugin_version AS key') && query.includes('action, count()'))
        return analyticsResponse([{ key: '8.1.0', action: 'download_fail', devices: '1' }])
      return analyticsResponse([])
    },
  })
