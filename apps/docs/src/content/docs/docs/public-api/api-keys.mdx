...
For CI that creates a temporary preview channel, uploads and promotes a bundle, then deletes the channel, bind `app_preview` only to the preview app:
...
```
`app_id` is the app record’s internal UUID, not the public app or bundle identifier (for example, `com.example.app`).

This role includes only `app.read`, `app.read_bundles`, `app.upload_bundle`, `app.create_channel`, `channel.read`, `channel.promote_bundle`, and `channel.delete`. It deliberately does not include `bundle.delete`, so preview cleanup must not use `channel delete --delete-bundle`.

The role is scoped to an app, not a pull request or channel name. It can delete any channel in the bound app. For the dashboard setup, see [Use an App Preview key for preview workflows](/docs/webapp/api-keys/#use-an-app-preview-key-for-preview-workflows).
...
2. **PR Preview Channels**: Use `app_preview` on only the preview app or apps when CI needs to upload and promote bundles, then create and delete temporary channels.
