...
How permissions are evaluated depends on whether your organization has RBAC enabled and whether the key has explicit role bindings (identified by a non-null `rbac_id`):

| Condition | What is evaluated |
|---|---|
| Legacy org (no RBAC) | Key mode capped on owner's org membership rights |
| RBAC org, key has no bindings | Owner's RBAC role bindings, capped by key mode |
| RBAC org, key has explicit bindings | **Only the key's own bindings** — owner's permissions are ignored |

The third case is the most important for security: a key with explicit bindings is strictly isolated from its owner's rights. This allows creating minimal-privilege automation keys even when the owner is a super admin.

See the [API Key permission system](/docs/webapp/api-key-permissions/) for the full explanation.
...
interface ApiKey {
  ...
  user_id: string
  rbac_id: string | null  // RBAC principal UUID, null for legacy keys
