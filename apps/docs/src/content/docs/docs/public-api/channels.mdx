...
- **updatePackage**: Control whether devices download a zip, a delta, or both (`all`, `zip`, `delta`, `zip_from_builtin`, or `delta_from_builtin`). See [Update package](/docs/webapp/channels/#update-package).
