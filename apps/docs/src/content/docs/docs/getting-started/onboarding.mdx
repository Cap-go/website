...
- **Delta updates** stay enabled. Instant apply (`always`, `atInstall`, `onLaunch`) should keep using `npx @capgo/cli@latest bundle upload --delta` so only changed files download and the user is not left waiting on a full zip
