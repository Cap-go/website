...
Starting with plugin version **v5.10.0**, **v6.25.0**, and **v7.25.0**, the device ID persists across normal reinstall paths. On Android, persistence depends on Android backup/restore preserving the plugin preferences; if Android backup is disabled, backup rules exclude those preferences, or app data is cleared, a new device ID is generated. Updating the app does not create a new device ID. Older plugin versions can create a new device ID after reinstall.
...
- **Current behavior**: The device ID persists across normal reinstall paths. It is stored in Keychain on iOS, secure storage on Electron, and Android plugin preferences that Android can restore from backup.
- **Android caveat**: If Android backup is disabled, backup rules exclude the plugin preferences, or app data is cleared, Android generates a new device ID.
- **Previous behavior** (before v6.25.0/v7.25.0): For privacy reasons related to Apple and Google store policies, the device ID was reset on each app reinstall, making it impossible to track the same device across reinstalls.
