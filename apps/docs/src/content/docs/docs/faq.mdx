...
Starting with plugin version **v5.10.0**, **v6.25.0**, and **v7.25.0**, the device ID persists across app reinstalls. Updating the app does not create a new device ID. Older plugin versions can create a new device ID after reinstall.

For test and development builds, turn off development-build and emulator delivery on production channels. See [Test Native Builds Without Live Updates](/docs/live-updates/testing-native-builds-without-live-updates/) for the exact settings.
...
The stats are calculated based on the number of [MAU](/docs/faq/#what-counts-as-a-mau-for-capgo) that have been installed on your devices.
