...
-   release\_version (versionName from AndroidManifest.xml or CFBundleShortVersionString from Info.plist or `capacitor.config.json` if set in [`CapacitorUpdater.version`](/docs/plugins/updater/settings/#version) )
...
The device list shows the latest known metadata for each device, such as the device ID, platform, plugin version, OS version, native version, channel, and installed bundle. Capgo updates this metadata when the app checks in, but repeated check-ins that report the same metadata may not change the row or its last update time.
