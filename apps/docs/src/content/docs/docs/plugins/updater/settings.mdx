...
<Aside type="caution">
When `publicKey` is configured, signature validation is fail-closed. The updater rejects any update it cannot decrypt and verify with that key, including Delta updates. An unsigned or invalid update cannot be installed as an unverified fallback.
</Aside>
