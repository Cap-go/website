...
- `minor` blocks when the target bundle has a different major or minor than the device native baseline (`version_build`).
- `patch` blocks when the target bundle has a different major, minor, or patch than `version_build`. It only allows prerelease/build metadata changes on the same `MAJOR.MINOR.PATCH` core.
