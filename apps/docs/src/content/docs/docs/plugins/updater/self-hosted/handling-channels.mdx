...
1. **Platform check**: Channel must allow the device's platform (`ios`, `android`, or `electron`)
2. **Device type check**:
...
Channel assignments work together with your [Update API Endpoint](/docs/plugins/updater/self-hosted/handling-updates/). When a device requests an update, check its channel assignment to determine which version to serve:
