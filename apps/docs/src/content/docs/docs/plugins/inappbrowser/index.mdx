...
| `postMessage` | Sends an event to the webview (in-app browser). Listen in the page with `window.addEventListener('messageFromNative', listenerFunc)`. Payloads must be JSON-serializable, and when `id` is omitted the message is broadcast to all open webviews. |
