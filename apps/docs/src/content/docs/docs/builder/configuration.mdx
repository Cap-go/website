...
| _(env only)_ | `PLAY_STORE_TRACK` | — | `internal` | Google Play release track (`internal`, `alpha`, `beta`, `production`) |
| _(env only)_ | `PLAY_STORE_RELEASE_STATUS` | — | `draft` | Release status on the chosen track (`draft`, `completed`, `inProgress`, `halted`) |
