...
For a kiosk flavour distributed through MDM, configure the Android project/build to produce an APK. Use `--output-upload` to retrieve the signed output, then distribute that APK through your MDM instead of Google Play.
