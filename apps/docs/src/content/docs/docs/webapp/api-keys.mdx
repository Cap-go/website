...
This selected-apps-only mode does not grant an organization-wide role. Each selected app binding remains associated with its owning organization, and the key is limited to the selected apps with only these permissions:
