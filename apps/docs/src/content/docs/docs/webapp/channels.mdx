...
Next is the `Update package` setting. It chooses whether devices download a full zip, a delta of changed files, or both. See [Update package](#update-package).
...
Eight is the `Allow devices to self associate`. If this is true then the [setChannel](/docs/plugins/updater/api/#setchannel) method will be available. If this is set to false and you try to call the [setChannel](/docs/plugins/updater/api/#setchannel) method with this channel then the call will fail.
