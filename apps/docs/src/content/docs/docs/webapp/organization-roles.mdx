...
| **App Preview** | `app_preview` | App-scoped preview CI lifecycle: upload and promote bundles, then create and delete channels. |
...
Use **App Preview** (`app_preview`) for an app-scoped CI key that manages the preview lifecycle without broad app or organization access.
...
| `channel.read` | Read a channel |
| `channel.promote_bundle` | Set a channel's active bundle |
| `channel.delete` | Delete a channel |

It does not include `app.update_settings`, device or role management, `channel.update_settings`, `channel.rollback_bundle`, forced-device management, or `bundle.delete`.

:::caution
**App Preview** is app-scoped, not channel-name or pull-request scoped. A key with this role can delete any channel in an app where it has this role.
:::
