...
SSO does not change what users can do. Permissions are controlled by [organization roles](/docs/webapp/organization-system/#permission-breakdown). SSO only changes how they authenticate.
...
You can **Deactivate** a provider at any time to pause SSO for that domain without deleting the configuration.
...
See the [full permission breakdown](/docs/webapp/organization-system/#permission-breakdown) for what each role can do.
...
On an **Active** provider you can toggle **Enforce SSO**. When enabled, Capgo marks existing auth users for that email domain as SSO-only and the login flow requires IdP authentication for that domain. Disabling the provider or turning enforcement off removes that SSO-only flag for the domain.
...
| **Disabled** | Provider paused; SSO not used; can be re-activated |
...
- [Organization system: roles and permissions](/docs/webapp/organization-system/)
