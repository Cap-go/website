...
2. **Cloud override**: If the device has been assigned to a channel from the dashboard or Public API, that override is used. Plugin [`setChannel()`](/docs/plugins/updater/api/#setchannel) on current versions is local and is a separate step. See [How a device picks a channel](/docs/live-updates/channels/#how-a-device-picks-a-channel-precedence).
