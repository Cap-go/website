...
:::caution
For least-privilege preview automation, use an app-scoped **App Preview** API key and omit `--delete-bundle`. That flag also requires `bundle.delete`, which **App Preview** intentionally does not grant. See [API Keys](/docs/webapp/api-keys/#use-an-app-preview-key-for-preview-workflows) for setup.
:::
