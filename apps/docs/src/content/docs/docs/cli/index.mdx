...
Always run Capgo CLI with `@latest` (for example `npx @capgo/cli@latest`). The CLI follows its own versioning and does **not** need to match your plugin major or Capacitor major. The latest CLI stays compatible with any supported updater plugin version. See [Capgo versioning](/docs/versioning/).
</Aside>
