...
* `--s3-endpoint <s3Endpoint>` URL of S3 endpoint. Does not work with delta uploads or the external option.
...
* `--delta` Uploads Delta (manifest) files alongside the full bundle.
* `--delta-only` Uploads only Delta (manifest) updates, skipping the full bundle.
* `--no-delta` Disables Delta (manifest) uploads (useful if an instant apply `autoUpdate` mode is enabled but you want a full bundle).
...
* `--disable-auto-update STRATEGY`  Disable auto update strategy for this channel. The possible options are: major, minor, patch, metadata, none.
* `--apikey [key]` API key to link to your account.
...
First, the `major` strategy. It prevents an update from native baseline `0.0.0` -> target bundle `1.0.0`. The major is the highlighted number (**1**.0.0 and **0**.0.0).\
...
In order for it to accept an update, the following conditions must be met:
 ...
 - The patch is the same between the new and the old version
 - Prerelease and build metadata may differ
...
The strategy comparison uses the native baseline sent as `version_build`, not the current downloaded bundle sent as `version_name`.
