...
- major: Block target bundles whose major version is above the device native baseline (`version_build`). Same-major minor and patch updates are still allowed.
- minor: Block target bundles whose major or minor version differs from the device native baseline. Patch and prerelease/build metadata changes inside the same major.minor line are allowed.
- patch: Very strict. Block target bundles whose major, minor, or patch number differs from the device native baseline. Only prerelease/build metadata changes on the same `MAJOR.MINOR.PATCH` core are allowed. Examples: `1.0.0-beta.1` -> `1.0.0-beta.2` allowed, `1.0.0` -> `1.0.1` blocked, `1.0.0` -> `1.1.0` blocked.
