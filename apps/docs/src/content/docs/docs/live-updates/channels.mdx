...
For a lasting assignment:

- Set `defaultChannel` in `capacitor.config.*` (requires a new native binary to change later).
- Call [`setChannel()`](/docs/plugins/updater/api/#setchannel) from the app. On plugin 5.34.0 / 6.34.0 / 7.34.0 / 8.0.0 and later, that assignment is local and is not removed by this cleanup.
...
- Update package: Choose whether devices download a full zip, a delta of changed files, or both (`all`, `zip`, `delta`, `zip_from_builtin`, `delta_from_builtin`). See [Update package](/docs/webapp/channels/#update-package) for the console dropdown and when each mode is useful.
