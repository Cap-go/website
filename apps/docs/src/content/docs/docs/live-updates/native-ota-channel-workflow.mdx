...
4. After that baseline upload, put `--fail-on-incompatible` back on normal OTA CI.
...
1. **One-time per channel: enable metadata gating**
   ...
   Repeat for `dev` if that channel also receives intentional native baselines.
...
4. **Resume guarded OTA uploads**
   ...
   Subsequent JS-only releases keep using `--fail-on-incompatible` again.

</Steps>

<Aside type="tip">
Treat the native-baseline upload as a **dedicated CI job** (or a manual release step), not as an exception you sprinkle into the default OTA job. The default job should always keep `--fail-on-incompatible`.
</Aside>
...
| --- | --- | --- |
| OTA | JS-only; native packages match the channel | `--fail-on-incompatible` |
| Native baseline | New native binary + matching JS bundle | **No** `--fail-on-incompatible`; use `--auto-min-update-version` (metadata strategy) |
