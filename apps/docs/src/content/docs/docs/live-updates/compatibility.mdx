...
<Aside type="caution">
Detecting incompatibility does **not** block the upload. If you upload a native-incompatible bundle and set it as a channel's active build, Capgo **will deliver it** to devices on that channel.
</Aside>
...
### Prevent incompatible deliveries automatically

The one strategy that actually looks at your native packages is the `metadata` update strategy combined with `--auto-min-update-version`. Switch the channel to that strategy once, then add the flag to your uploads — Capgo runs the compatibility check on every upload and, when a bundle needs new native code, raises the update floor for you so devices that haven't installed the matching native build simply don't receive it:

```bash
# one-time: switch the channel to the metadata strategy
npx @capgo/cli channel set production com.example.app --disable-auto-update metadata
...
<Aside type="caution">
Version-number rules like `--disable-auto-update major` are **not** a substitute — they only block version jumps and never inspect native code, so a native change shipped in a minor or patch release still goes out. Only `--auto-min-update-version` actually checks your native packages.
</Aside>
