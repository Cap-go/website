...
The updater plugin can call two separate endpoints:

- `updateUrl` (`/updates`) checks which bundle the device should receive. This endpoint is required for Capgo Cloud live updates.
- `statsUrl` (`/stats`) sends explicit update lifecycle, health, and failure events. Set `statsUrl` to an empty string to disable these extra stats reports, or point it to an endpoint or proxy you control to keep those explicit statistics in your infrastructure. See [HIPAA Compliance](/docs/live-updates/hipaa-compliance/#option-3-self-host-the-statistics-endpoint) for the self-hosted endpoint contract.
