...
- Apps that need EU data residency can set the updater plugin's `updateUrl`, `statsUrl`, and `channelUrl` to the EU Capgo Cloud host. See [Data Location](/docs/live-updates/data-location/) for the exact configuration.
