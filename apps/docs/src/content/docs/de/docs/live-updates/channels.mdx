...
<Aside type="caution" title="Kanäle bieten keine Vertraulichkeit">
Kanäle steuern die Update-Berechtigung, nicht die Geheimhaltung des Bundles. Selbst wenn ein Kanal privat ist oder Self-Assignment deaktiviert wurde, sollte jedes unverschlüsselte zu Capgo hochgeladene Bundle weiterhin als öffentliches an Clients ausgeliefertes Asset behandelt werden. Die Verschlüsselung schützt den Auslieferungsweg und die Authentizität von Updates, aber ausgelieferte Bundles können mit genügend Aufwand weiterhin aus der verteilten App rückentwickelt werden, weil der öffentliche Schlüssel Teil des Clients ist. Siehe [Live-Update-Verschlüsselung](/docs/live-updates/encryption/) für Details.
</Aside>
